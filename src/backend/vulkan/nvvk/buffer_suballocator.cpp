...
VkResult BufferSubAllocator::init(const InitInfo& info)
{
  ...
  m_state.maxAllocationSize =
      std::min((VkDeviceSize(1) << (sizeof(BufferSubAllocation::size)) * 8) - 1,
  ...
  size_t maxBlocks =
      (m_info.maxAllocatedSize + m_info.blockSize - 1) / m_info.blockSize;
  assert(maxBlocks <= MAX_TOTAL_BLOCKS);

  m_state.maxBlocks = static_cast<uint32_t>(maxBlocks);
  m_state.internalBlockUnits = static_cast<uint32_t>(
      (m_info.blockSize + m_info.minAlignment - 1) / m_info.minAlignment);

  if (m_info.keepLastBlock)
  {
    Block block;
    block.offsetAllocator = std::make_unique<OffsetAllocator::Allocator>(
        m_state.internalBlockUnits, m_info.perBlockAllocations);
    NVVK_FAIL_RETURN(createNewBuffer(block.buffer,
                                     VkDeviceSize(m_state.internalBlockUnits) *
                                         m_info.minAlignment,
...
BufferSubAllocator::Report BufferSubAllocator::getReport() const
{
  ...
  for (size_t i = 0; i < m_blocks.size(); i++)
  {
    ...
    const OffsetAllocator::Allocator* offsetAllocator =
        block.offsetAllocator.get();

    if (offsetAllocator)
    {
      OffsetAllocator::StorageReport storageReport =
          offsetAllocator->storageReport();
      report.reservedSize += VkDeviceSize(m_state.internalBlockUnits -
                                          storageReport.totalFreeSpace) *
                             m_info.minAlignment;
      report.freeSize =
          VkDeviceSize(storageReport.totalFreeSpace) * m_info.minAlignment;
    }
    else
    {
      // dedicated blocks
...
VkResult BufferSubAllocator::subAllocate(BufferSubAllocation& subAllocation,
                                         VkDeviceSize size, uint32_t alignment)
{
  subAllocation = {};
  ...
  {
    if (m_state.freeBlockIndex == INVALID_BLOCK_INDEX &&
        m_blocks.size() == size_t(m_state.maxBlocks))
    {
      return VK_ERROR_OUT_OF_DEVICE_MEMORY;
    }
    ...
    if (!alignmentIsPowerOfTwo)
    {
      ...
      for (uint32_t searchAlignment = MIN_ALIGNMENT;
           searchAlignment <= MAX_ALIGNMENT; searchAlignment *= 2)
      {
    ...
    NVVK_FAIL_RETURN(createNewBuffer(block.buffer, size,
                                     std::max(m_info.minAlignment, alignment),
                                     freeBlockIndex));
  ...
  if (!alignmentIsPowerOfTwo || alignment > m_info.minAlignment)
  {
    ...
    // The offset returned from OffsetAllocator will only be aligned to
    // m_info.minAlignment. With the extra safety margin space, we can later
    // adjust the returned offset to alignment, see logic in `subRange`.
    sizeAllocate = (sizeAllocate + alignment - 1);
  }

  // offset allocator works in units of `m_info.minAlignment`
  uint32_t allocatorUnits = static_cast<uint32_t>(
      (sizeAllocate + m_info.minAlignment - 1) / m_info.minAlignment);
  ...
  while (activeBlockIndex != INVALID_BLOCK_INDEX)
  {
    ...
    OffsetAllocator::Allocation allocation =
        block.offsetAllocator->allocate(allocatorUnits);
...
void BufferSubAllocator::subFree(BufferSubAllocation& subAllocation)
{
  ...
  OffsetAllocator::Allocator* offsetAllocator =
      m_blocks[subAllocation.block].offsetAllocator.get();
  ...
  if (!offsetAllocator || offsetAllocator->storageReport().totalFreeSpace ==
                              m_state.internalBlockUnits)
  {
    // always free if dedicated
    // and maybe depending if we are the last one
    if (!offsetAllocator ||
        (m_state.activeBlockCount > 1 || !m_info.keepLastBlock))
    {
      m_info.resourceAllocator->destroyBuffer(
          m_blocks[subAllocation.block].buffer);
...
BufferRange
BufferSubAllocator::subRange(const BufferSubAllocation& subAllocation) const
{
  // make it legal to pass unset ranges
  if (!subAllocation)
  {
  ...
  // The original requested alignment might have been greater than the
  // minAlignment, or might be non-power-of-two. In that case we need to
  // re-adjust the offset, which is safe to work as we allocated a safety
  // margin.
...
VkResult BufferSubAllocator::createNewBuffer(nvvk::Buffer& buffer,
                                             VkDeviceSize size,
                                             uint32_t alignment,
                                             uint32_t blockIndex)
{
  NVVK_FAIL_RETURN(m_info.resourceAllocator->createBuffer(
      buffer, size, m_info.usageFlags, m_info.memoryUsage,
      m_info.allocationFlags, alignment, m_info.queueFamilies));

  nvvk::DebugUtil::getInstance().setObjectName(
      buffer.buffer, std::string(typeid(*this).name()) + "::" +
                         m_info.debugName + "_" + std::to_string(blockIndex));
  return VK_SUCCESS;
}
...
[[maybe_unused]] static void usage_BufferSubAllocator()
{
  ...
  bufferSubAllocator.init(
      {.debugName = "meshes",
       .usageFlags = VK_BUFFER_USAGE_2_INDEX_BUFFER_BIT |
                     VK_BUFFER_USAGE_2_VERTEX_BUFFER_BIT,
       .memoryUsage = VMA_MEMORY_USAGE_AUTO_PREFER_DEVICE,
       .blockSize =
           64 * 1024 *
           1024});  // allocations greater blockSize get their own block
  ...
  while (true)
  {
    ...
    for (Mesh& mesh : meshes)
    {
      ...
      if (vertexRange.buffer != lastVertexBuffer)
      {
        ...
        vkCmdBindVertexBuffers2(cmd, 0, 1, &vertexRange.buffer, &offset, &size,
                                &stride);
      ...
      // need to apply the buffer offsets to the draw given we bound the full
      // buffer
      vkCmdDrawIndexed(cmd, uint32_t(mesh.indexSize / sizeof(uint16_t)), 1U,
                       uint32_t(indexRange.offset / sizeof(uint16_t)),
