...
{
  return VkExtent2D{(size.width + (workgroupSize - 1)) / workgroupSize,
                    (size.height + (workgroupSize - 1)) / workgroupSize};
}
...
inline VkExtent3D getGroupCounts(const VkExtent3D& size,
