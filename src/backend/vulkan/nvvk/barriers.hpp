...
//
[[nodiscard]] constexpr VkAccessFlags2
inferAccessMaskFromStage(VkPipelineStageFlags2 stage, bool read)
{
  ...
  {
    access |= read ? VK_ACCESS_2_SHADER_READ_BIT | VK_ACCESS_2_UNIFORM_READ_BIT
  ...
  if ((stage & VK_PIPELINE_STAGE_2_TRANSFER_BIT) != 0)
    access |=
        read ? VK_ACCESS_2_TRANSFER_READ_BIT : VK_ACCESS_2_TRANSFER_WRITE_BIT;
  ...
  assert((access != 0 || stage == VK_PIPELINE_STAGE_2_NONE) &&
...
//
[[nodiscard]] constexpr std::tuple<VkPipelineStageFlags2, VkAccessFlags2>
inferPipelineStageAccessTuple(VkImageLayout state)
{
  switch (state)
  {
...
struct ImageMemoryBarrierParams
{
  ...
  VkPipelineStageFlags2 srcStageMask =
      INFER_BARRIER_PARAMS;  // infer from oldLayout
  VkPipelineStageFlags2 dstStageMask =
      INFER_BARRIER_PARAMS;  // infer from newLayout
  VkAccessFlags2 srcAccessMask =
      INFER_BARRIER_PARAMS;  // infer from stage or layout
...
makeImageMemoryBarrier(const ImageMemoryBarrierParams& params)
{
  ...
  if (params.srcStageMask == INFER_BARRIER_PARAMS &&
      params.srcAccessMask == INFER_BARRIER_PARAMS)
  {
    const auto [srcStageMask, srcAccessMask] =
        inferPipelineStageAccessTuple(params.oldLayout);
  ...
  if (params.dstStageMask == INFER_BARRIER_PARAMS &&
      params.dstAccessMask == INFER_BARRIER_PARAMS)
  {
    const auto [dstStageMask, dstAccessMask] =
        inferPipelineStageAccessTuple(params.newLayout);
  ...
  else if (params.dstAccessMask == INFER_BARRIER_PARAMS)
  {
    ...
    barrier.dstAccessMask =
        inferAccessMaskFromStage(params.dstStageMask, false);
  }
...
// In the pipeline, the image must be in the correct layout to be used, and this
// function is used to transition the image to the correct layout.
//
void cmdImageMemoryBarrier(VkCommandBuffer cmd,
                           const ImageMemoryBarrierParams& params);
...
void cmdImageMemoryBarrier(VkCommandBuffer cmd, nvvk::Image& image,
                           const ImageMemoryBarrierParams& params);
...
struct BufferMemoryBarrierParams
{
  ...
  VkAccessFlags2 srcAccessMask =
      INFER_BARRIER_PARAMS;  // infers from srcStageMask
  VkAccessFlags2 dstAccessMask =
      INFER_BARRIER_PARAMS;  // infers from dstStageMask
  uint32_t srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
  uint32_t dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
};
...
// Provide explicit access masks for fine-grained control, or use
// INFER_BARRIER_PARAMS for automatic inference
//
[[nodiscard]] constexpr VkBufferMemoryBarrier2
makeBufferMemoryBarrier(const BufferMemoryBarrierParams& params)
{
...
void cmdBufferMemoryBarrier(VkCommandBuffer commandBuffer,
                            const BufferMemoryBarrierParams& params);

[[nodiscard]] constexpr VkMemoryBarrier2
makeMemoryBarrier(VkPipelineStageFlags2 srcStageMask,
                  VkPipelineStageFlags2 dstStageMask,
                  VkAccessFlags2 srcAccessMask =
                      INFER_BARRIER_PARAMS,  // Default to infer if not provided
                  VkAccessFlags2 dstAccessMask =
                      INFER_BARRIER_PARAMS  // Default to infer if not provided
)
{
  const VkMemoryBarrier2 memoryBarrier{
      ...
      .srcAccessMask = (srcAccessMask != INFER_BARRIER_PARAMS)
                           ? srcAccessMask
                           : inferAccessMaskFromStage(srcStageMask, false),
      .dstStageMask = dstStageMask,
      .dstAccessMask = (dstAccessMask != INFER_BARRIER_PARAMS)
                           ? dstAccessMask
                           : inferAccessMaskFromStage(dstStageMask, true)};
  return memoryBarrier;
}

void cmdMemoryBarrier(
    VkCommandBuffer cmd, VkPipelineStageFlags2 srcStageMask,
    VkPipelineStageFlags2 dstStageMask,
    VkAccessFlags2 srcAccessMask =
        INFER_BARRIER_PARAMS,  // Default to infer if not provided
    VkAccessFlags2 dstAccessMask =
...
public:
  ...
  void cmdPipelineBarrier(VkCommandBuffer cmd,
  ...
  void appendOptionalLayoutTransition(nvvk::Image& image,
