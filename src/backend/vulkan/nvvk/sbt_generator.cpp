...
//--------------------------------------------------------------------------------------------------
// Finding the handle index position of each group type in the pipeline creation
// info. If the pipeline was created like: raygen, miss, hit, miss, hit, hit The
// result will be: raygen[0], miss[1, 3], hit[2, 4, 5], callable[]
//
void SBTGenerator::addIndices(
  ...
  // groups are used in the final pipeline we need to offset them to ensure each
  // group has a unique index
...
//--------------------------------------------------------------------------------------------------
// Calculates the required size for the Shader Binding Table (SBT) buffer for a
// ray tracing pipeline. Analyzes the pipeline and libraries to determine group
// counts, strides, and alignment. Updates internal state for SBT layout and
// returns the total buffer size needed. Call after pipeline creation; use
// result to allocate the SBT buffer.
//
size_t SBTGenerator::calculateSBTBufferSize(
  ...
  // Special case: Raygen group stride must be aligned to
  // shaderGroupBaseAlignment
  m_stride[eRaygen] =
      nvutils::align_up(m_stride[eRaygen], m_shaderGroupBaseAlignment);
  ...
  totalSize += nvutils::align_up(
      m_stride[eRaygen] * getGroupIndexCount(eRaygen), bufferAlignment);
  m_bufferAddresses[eMiss] = totalSize;
  totalSize += nvutils::align_up(m_stride[eMiss] * getGroupIndexCount(eMiss),
                                 bufferAlignment);
  m_bufferAddresses[eHit] = totalSize;
  totalSize += nvutils::align_up(m_stride[eHit] * getGroupIndexCount(eHit),
                                 bufferAlignment);
  m_bufferAddresses[eCallable] = totalSize;
  totalSize += nvutils::align_up(
      m_stride[eCallable] * getGroupIndexCount(eCallable), bufferAlignment);
...
//--------------------------------------------------------------------------------------------------
// Fills the SBT buffer with shader group handles and any attached data for the
// ray tracing pipeline. Requires a buffer of the correct size and alignment, as
// calculated by calculateSBTBufferSize(). The buffer must be created with
...
VkResult SBTGenerator::populateSBTBuffer(VkDeviceAddress bufferAddress,
  ...
  NVVK_FAIL_RETURN(vkGetRayTracingShaderGroupHandlesKHR(
      m_device, m_pipeline, 0, m_totalGroupCount, sbtSize,
  ...
  copyHandles(m_bufferAddresses[eRaygen], m_shaderGroupIndices[eRaygen],
              m_stride[eRaygen], m_data[eRaygen]);
  copyHandles(m_bufferAddresses[eMiss], m_shaderGroupIndices[eMiss],
              m_stride[eMiss], m_data[eMiss]);
  copyHandles(m_bufferAddresses[eHit], m_shaderGroupIndices[eHit],
              m_stride[eHit], m_data[eHit]);
  copyHandles(m_bufferAddresses[eCallable], m_shaderGroupIndices[eCallable],
              m_stride[eCallable], m_data[eCallable]);
...
[[maybe_unused]] static void usage_SBTGenerator() {
  ...
  VkPhysicalDeviceRayTracingPipelinePropertiesKHR rtProp{
  ...
  NVVK_CHECK(vkCreateRayTracingPipelinesKHR(device, {}, {}, 1, &rayPipelineInfo,
                                            nullptr, &rtPipeline));
  ...
  size_t bufferSize =
      sbtGenerator.calculateSBTBufferSize(rtPipeline, rayPipelineInfo);

  // Create SBT buffer using the size from above
  NVVK_CHECK(allocator.createBuffer(
      sbtBuffer, bufferSize, VK_BUFFER_USAGE_2_SHADER_BINDING_TABLE_BIT_KHR,
  ...
  NVVK_CHECK(sbtGenerator.populateSBTBuffer(sbtBuffer.address, bufferSize,
                                            sbtBuffer.mapping));

  // Retrieve the regions, which are using addresses based on the
  // m_sbtBuffer.address
  ...
  // ##Special case
  // The SBT can be created with only a few groups, but the pipeline can have
  // many more groups. It is also possible to create a pipeline with only a few
  // groups but having a SBT representing many more groups

  // The following example shows a more complex setup.There are : 1 x raygen,
  // 2 x miss, 2 x hit.BUT the SBT will have 3 hit by duplicating the second hit
  // in its table.So, the same hit shader defined in the pipeline, can be called
  // with different data.
  //
  // In this case,
  // the use must provide manually the information to the SBT.All extra group
  // must be explicitly added.
  //
  // The following show how to get handle indices provided in the pipeline,
  // and we are adding another hit group, re - using the 4th pipeline
  // entry.Note: we are not providing the pipelineCreateInfo, because we are
