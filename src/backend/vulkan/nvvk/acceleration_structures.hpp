...
This function sets up a memory barrier specifically for acceleration structure
operations in Vulkan, ensuring proper data synchronization during the build or
update phases. It operates between pipeline stages that deal with acceleration
structure building.

# `nvvk::toTransformMatrixKHR` Function
This function converts a `glm::mat4` matrix to the matrix format required by
acceleration structures in Vulkan.

# `nvvk::AccelerationStructureGeometryInfo` Structure
- **Purpose**: Holds information about acceleration structure geometry,
including the geometry structure and build range information.

# `nvvk::AccelerationStructureBuildData` Structure
- **Purpose**: Manages the building of Vulkan acceleration structures of a
specified type.
- **Key Functions**:
  - `addGeometry`: Adds a geometry with build range information to the
acceleration structure.
  - `finalizeGeometry`: Configures the build information and calculates the
necessary size information.
  - `makeCreateInfo`: Creates an acceleration structure based on the current
build and size information.
  - `cmdBuildAccelerationStructure`: Builds the acceleration structure in a
Vulkan command buffer.
  - `cmdUpdateAccelerationStructure`: Updates the acceleration structure in a
Vulkan command buffer.
  - `hasCompactFlag`: Checks if the compact flag is set for the build.
- **Usage**:
  - For each BLAS,
    - Add geometry using `addGeometry`.
    - Finalize the geometry and get the size requirements using
`finalizeGeometry`.
...
inline void accelerationStructureBarrier(VkCommandBuffer cmd, VkAccessFlags src,
                                         VkAccessFlags dst)
{
  assert(src == VK_ACCESS_TRANSFER_WRITE_BIT ||
         src == VK_ACCESS_ACCELERATION_STRUCTURE_WRITE_BIT_KHR);
  assert(dst == VK_ACCESS_ACCELERATION_STRUCTURE_READ_BIT_KHR ||
         dst == VK_ACCESS_ACCELERATION_STRUCTURE_WRITE_BIT_KHR ||
         dst == (VK_ACCESS_ACCELERATION_STRUCTURE_WRITE_BIT_KHR |
  ...
  vkCmdPipelineBarrier(
      cmd,
      src == VK_ACCESS_TRANSFER_WRITE_BIT
...
struct AccelerationStructureBuildData
{
  VkAccelerationStructureTypeKHR asType =
      VK_ACCELERATION_STRUCTURE_TYPE_MAX_ENUM_KHR;  // Mandatory to set
  ...
  // Adds a geometry with its build range information to the acceleration
  // structure.
  ...
  makeInstanceGeometry(size_t numInstances, VkDeviceAddress instanceBufferAddr);

  // Configures the build information and calculates the necessary size
  // information.
  VkAccelerationStructureBuildSizesInfoKHR
  finalizeGeometry(VkDevice device, VkBuildAccelerationStructureFlagsKHR flags);
  ...
  void cmdUpdateAccelerationStructure(
      VkCommandBuffer cmd, VkAccelerationStructureKHR accelerationStructure,
      VkDeviceAddress scratchAddress);
  ...
  {
    return buildInfo.flags &
           VK_BUILD_ACCELERATION_STRUCTURE_ALLOW_COMPACTION_BIT_KHR;
  }
};

// Get the maximum scratch buffer size required for the acceleration structure
// build
...
/*-------------------------------------------------------------------------------------------------
 ...
 Manages the construction and optimization of bottom-level acceleration
 structures (BLAS)

 This class is designed to facilitate the construction of BLAS based on provided
 build information and queries.
  - Compacts BLAS for efficient memory usage and cleans up resources.
  - It ensures that operations are performed within a specified memory budget if
 possible.
 ...
 - Query the scratch buffer size via `getScratchSize` (providing a reasonable
 budget allows to run more builds in parallel)
 - Create scratch buffer

 Within a loop
   - Call `cmdCreateBlas` to create all or a range of BLAS that are provided
 BlasBuildData
   - Submit command buffer and wait
   - Call `cmdCompactBlas` to compact the BLAS that have been built thus far.
   - Call `destroyNonCompactedBlas` to destroy the original BLAS that were
 compacted.
...
public:
  ...
  // The advantage of this function is that it will try to build as many BLAS as
  // possible in parallel as the `scratchSize` and `hintMaxBudget` allows.
  // Higher `scratchSize` allows less barriers, and higher `hintMaxBudget` means
  // less need to call this function multiple times.
  //
  // Returns VK_SUCCESS if the entire input vector was processed,
  // returns VK_INCOMPLETE if this function needs to be called again until it
  // returns VK_SUCCESS Other result codes are to be treated as error.
  VkResult cmdCreateBlas(
      VkCommandBuffer cmd,
      std::span<AccelerationStructureBuildData>&
          blasBuildData,  // List of the BLAS to build */
      std::span<nvvk::AccelerationStructure>&
          blasAccel,                   // List of the acceleration structure
      ...
      VkDeviceSize hintMaxBudget =
          512'000'000  // Ceiling for sum of acceleration structure sizes built
  );

  // Compact the BLAS that have been built
  // Synchronization must be done by the application between the build and the
  // compact
  VkResult
  cmdCompactBlas(VkCommandBuffer cmd,
  ...
  // - If the accumulated size is smaller or equal the budget, return it. This
  // will allow to build all BLAS in one iteration.
  // - If the maximum size is greater than the budget, return it, so building
  // the largest BLAS is guaranteed.
  // - Else return budget.
  //
  // Usage
...
private:
  AccelerationStructureBuilder&
  operator=(const AccelerationStructureBuilder&) = default;

  void destroy();
  void destroyQueryPool();
  VkResult initializeQueryPoolIfNeeded(
      VkCommandBuffer cmd,
      const std::span<AccelerationStructureBuildData>& blasBuildData);
  VkResult cmdBuildAccelerationStructures(
      VkCommandBuffer cmd,
      VkDeviceSize& budgetUsed,  // in/out: budget used so far
      std::span<AccelerationStructureBuildData>& blasBuildData,
      std::span<nvvk::AccelerationStructure>& blasAccel,
      VkDeviceAddress scratchAddress, VkDeviceAddress scratchAddressEnd,
      VkDeviceSize hintMaxBudget, uint32_t& currentQueryIdx,
  ...
  nvvk::ResourceAllocator*
      m_alloc{};        // Allocator for the creation of acceleration structures
  ...
  std::queue<CompactBatchInfo>
      m_batches;  // Queue of compact batches to be processed

  std::vector<nvvk::AccelerationStructure>
      m_cleanupBlasAccel;  // List of BLAS to be cleaned up

  // Stats
  Stats m_stats;  // Statistics about the compacted BLAS
};

// Helper class for building both Bottom-Level Acceleration Structures (BLAS)
// and Top-Level Acceleration Structures (TLAS). This utility abstracts the
// complexity of acceleration structure generation while allowing compacting,
// updating, and managing buffers. For more advanced control one shall use
// nvvk::BlasBuilder and nvvk::AccelerationStructureBuildData.
class AccelerationStructureHelper
{
public:
  ~AccelerationStructureHelper()
  {
    assert(!m_transientPool && "deinit missing");
  }
...
public:
  ...
  void blasSubmitBuildAndWait(
      const std::vector<nvvk::AccelerationStructureGeometryInfo>& asGeoInfoSet,
...
public:
  ...
  // add VK_BUILD_ACCELERATION_STRUCTURE_ALLOW_UPDATE_BIT_KHR to buildFlags if
  // you intend to use tlasUpdate
  void tlasSubmitBuildAndWait(
  ...
  // If instance count differs from original, a rebuild is performed instead of
  // an update. TLAS must have been built with the
  // VK_BUILD_ACCELERATION_STRUCTURE_ALLOW_UPDATE_BIT_KHR flag.
  void tlasSubmitUpdateAndWait(
      const std::vector<VkAccelerationStructureInstanceKHR>& tlasInstances);
