                                         ...
                                         uint32_t queueFamilyIndex)
{
  ...
  const VkCommandPoolCreateInfo commandPoolCreateInfo{
      ...
      .flags = VK_COMMAND_POOL_CREATE_TRANSIENT_BIT,  // Hint that commands will
                                                      // be short-lived
                               ...
                               VkCommandPool cmdPool, VkQueue queue)
{
  ...
  NVVK_FAIL_RETURN(vkQueueSubmit2(queue, uint32_t(submitInfo.size()),
                                  submitInfo.data(), fence[0]));
  NVVK_FAIL_RETURN(vkWaitForFences(device, uint32_t(fence.size()), fence.data(),
                                   VK_TRUE, UINT64_MAX));
