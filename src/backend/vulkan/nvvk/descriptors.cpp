...
DescriptorBindings::getWriteSet(uint32_t binding, VkDescriptorSet dstSet,
  ...
  assert(writeSet.dstArrayElement + writeSet.descriptorCount <=
         b.descriptorCount);
...
VkResult DescriptorBindings::createDescriptorSetLayout(
  ...
  result = vkCreateDescriptorSetLayout(device, &createInfo, nullptr,
...
VkResult DescriptorPack::init(const DescriptorBindings &bindings,
  ...
  if (numSets > 0) {
    ...
    NVVK_FAIL_RETURN(
        vkCreateDescriptorPool(device, &poolCreateInfo, nullptr, &m_pool));
    ...
    NVVK_FAIL_RETURN(
        vkAllocateDescriptorSets(device, &allocInfo, m_sets.data()));
  }
...
[[maybe_unused]] static void usage_DescriptorBindings() {
  ...
  // Manually create layout and pool
  {
    ...
    bindings.addBinding(SHADERIO_BINDING, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1,
    ...
    // To manually create a layout and a pool and to allocate NUM_SETS sets:
    {
      ...
      std::vector<VkDescriptorPoolSize> poolSizes =
          bindings.calculatePoolSizes(NUM_SETS);
      // Or if you have multiple descriptor layouts you'd like to allocate from
      // a single pool, you can use bindings.appendPoolSizes().

      VkDescriptorPool dpool = VK_NULL_HANDLE;
      const VkDescriptorPoolCreateInfo dpoolInfo{
          .sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO,
          .maxSets = NUM_SETS,
  ...
  nvvk::DescriptorPack dpack;
  {
    ...
    bindings.addBinding(SHADERIO_BINDING, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1,
                        ...
                        VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT |
                            VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT);
    // The second argument, NUM_SETS here, can be left 0 when the intend is to
    // use push descriptors, it defaults to `1`
    NVVK_CHECK(dpack.init(bindings, device, NUM_SETS));
  }

  // To update DescriptorSets:
  {
    // std::vectors inside for VkWriteDescriptorSets as well as the
    // corresponding payloads.
    nvvk::WriteSetContainer writeContainer;
    // makeDescriptorWrite returns a VkWriteDescriptorSet without actual binding
    // information, the append function takes care of that.

    // when preparing push descriptors, `dpack.sets[0]` would be omitted /
    // nullptr
    writeContainer.append(dpack.makeWrite(SHADERIO_BINDING, 0), myBufferA);

    // shortcut to provide `dpack.sets[1]` (also works when dpack.sets.empty()
    // for push descriptors)
    writeContainer.append(dpack.makeWrite(SHADERIO_BINDING, 1), myBufferB);

    vkUpdateDescriptorSets(device, writeContainer.size(), writeContainer.data(),
                           0, nullptr);

    // the writeContainer can also be used for push descriptors, when the
    // VkDescriptorSet provided were nullptr vkCmdPushDescriptorSet(cmd,
    // VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0,
    // writeContainer.size(), writeContainer.data());
  }

  // To create a pipeline layout with an additional push constant range:
  VkPushConstantRange pushRange{.stageFlags = VK_SHADER_STAGE_ALL,
                                .offset = 0,
                                .size = sizeof(PushConstants)};
  VkPipelineLayout pipelineLayout = VK_NULL_HANDLE;
  NVVK_CHECK(nvvk::createPipelineLayout(device, &pipelineLayout,
                                        {dpack.getLayout()}, {pushRange}));
