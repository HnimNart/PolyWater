...
// This class is used to create and store samplers, and to avoid creating the
// same sampler multiple times.
//
// Usage:
...
private:
  ...
  struct SamplerState
  {
    ...
    bool operator==(const SamplerState& other) const
    {
      return other.createInfo.flags == createInfo.flags &&
             ...
             other.createInfo.mipmapMode == createInfo.mipmapMode &&
             other.createInfo.addressModeU == createInfo.addressModeU &&
             other.createInfo.addressModeV == createInfo.addressModeV &&
             other.createInfo.addressModeW == createInfo.addressModeW &&
             other.createInfo.mipLodBias == createInfo.mipLodBias &&
             ...
             other.createInfo.compareOp == createInfo.compareOp &&
             other.createInfo.minLod == createInfo.minLod &&
             other.createInfo.maxLod == createInfo.maxLod &&
             other.createInfo.borderColor == createInfo.borderColor &&
             other.createInfo.unnormalizedCoordinates ==
             ...
             other.ycbr.forceExplicitReconstruction ==
                 ycbr.forceExplicitReconstruction;
    }
  };
  ...
  struct SamplerStateHashFn
  {
    std::size_t operator()(const SamplerState& s) const
    {
      return core::hashVal(
          ...
          s.createInfo.compareOp, s.createInfo.minLod, s.createInfo.maxLod,
          s.createInfo.borderColor, s.createInfo.unnormalizedCoordinates,
          s.reduction.reductionMode, s.ycbr.format, s.ycbr.ycbcrModel,
          s.ycbr.ycbcrRange, s.ycbr.components.r, s.ycbr.components.g,
          s.ycbr.components.b, s.ycbr.components.a, s.ycbr.xChromaOffset,
  ...
  // Stores unique samplers with their corresponding VkSamplerCreateInfo and
  // reference counts
  std::unordered_map<SamplerState, SamplerEntry, SamplerStateHashFn>
      m_samplerMap{};

  // Reverse lookup map for O(1) sampler release - must stay in sync with
  // m_samplerMap
