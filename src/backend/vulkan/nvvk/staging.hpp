...
public:
  ...
  ~StagingUploader()
  {
    assert(isAppendedEmpty() &&
           "Did you forget cmdUploadAppended() or cancelAppended()");
    assert(m_resourceAllocator == nullptr && "Missing deinit()");
  }

  // explicit lifetime of resourceAllocator must be ensured externally
  void init(ResourceAllocator* resourceAllocator,
  ...
  // All temporary staging resources are associated with the provided
  // SemaphoreState.
  ...
  // virtual so derived classes can implement staging space management by
  // different means.
  ...
  // returns staging buffer information that can be used for any manual copy
  // operations. if data is non-null it will be copied to bufferMapping
  // automatically virtual so that derived classes can implement this by
  // different means.
  virtual VkResult
  acquireStagingSpace(BufferRange& stagingSpace, size_t dataSize,
                      const void* data,
  ...
  VkResult appendBuffer(const nvvk::Buffer& buffer, VkDeviceSize bufferOffset,
                        VkDeviceSize dataSize, const void* data,
                        const SemaphoreState& semaphoreState = {});

  // same as above but infers `bufferOffset` from `bufferRange.offset` and
  // `dataSize` from `bufferRange.range`
  VkResult appendBufferRange(const nvvk::BufferRange& bufferRange,
                             const void* data,
                             const SemaphoreState& semaphoreState = {});
  ...
  // `dataSize` can be `0` does return VK_SUCCESS and sets `uploadMapping` to
  // nullptr
  VkResult appendBufferMapping(const nvvk::Buffer& buffer,
                               VkDeviceSize bufferOffset, VkDeviceSize dataSize,
                               void*& uploadMapping,
                               const SemaphoreState& semaphoreState = {});

  // same as above but infers `bufferOffset` from `bufferRange.offset` and
  // `dataSize` from `bufferRange.range`
  VkResult appendBufferRangeMapping(const nvvk::BufferRange& bufferRange,
                                    void*& uploadMapping,
                                    const SemaphoreState& semaphoreState = {});

  template <typename T>
  inline VkResult appendBuffer(const nvvk::Buffer& buffer, size_t bufferOffset,
                               std::span<T> data,
                               const SemaphoreState& semaphoreState = {})
  {
    return appendBuffer(buffer, bufferOffset, data.size_bytes(), data.data(),
                        semaphoreState);
  }

  template <typename T>
  inline VkResult appendBufferMapping(const nvvk::Buffer& buffer,
                                      size_t bufferOffset, size_t dataSize,
                                      T*& uploadMapping,
                                      const SemaphoreState& semaphoreState = {})
  {
    return appendBufferMapping(buffer, bufferOffset, dataSize,
                               (void*&) uploadMapping, semaphoreState);
  }

  template <typename T>
  inline VkResult appendBufferRange(const nvvk::BufferRange& bufferRange,
                                    std::span<T> data,
                                    const SemaphoreState& semaphoreState = {})
  {
  ...
  template <typename T>
  inline VkResult
  appendBufferRangeMapping(const nvvk::BufferRange& bufferRange,
                           T*& uploadMapping,
                           const SemaphoreState& semaphoreState = {})
  {
    return appendBufferRangeMapping(bufferRange, (void*&) uploadMapping,
                                    semaphoreState);
  }
  ...
  // if `imageTex.descriptor.imageLayout` is not
  // (VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL or
  //                                              VK_IMAGE_LAYOUT_GENERAL or
  //                                              VK_IMAGE_LAYOUT_SHARED_PRESENT_KHR)
  // then we transition to `VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL` prior any copy
  // operation in the appended batch. After the copy operations a transition is
  // added back to the image's original layout unless a valid `newLayout` is
  // provided. The `imageTex.descriptor.imageLayout` can also be updated by
  // providing `newLayout != VK_IMAGE_LAYOUT_UNDEFINED` through the post
  // operation barriers.

  // all image textures must only use the color aspect
  // this uploads mip 0/layer 0 only
  VkResult appendImage(nvvk::Image& image, size_t dataSize, const void* data,
                       VkImageLayout newLayout = VK_IMAGE_LAYOUT_UNDEFINED,
                       const SemaphoreState& semaphoreState = {});

  template <typename T>
  inline VkResult
  appendImage(nvvk::Image& image, std::span<T> data,
              VkImageLayout newLayout = VK_IMAGE_LAYOUT_UNDEFINED,
              const SemaphoreState& semaphoreState = {})
  {
    return appendImage(image, data.size_bytes(), data.data(), newLayout,
                       semaphoreState);
  }

  // subresource variant
  VkResult appendImageSub(nvvk::Image& image, const VkOffset3D& offset,
  ...
  template <typename T>
  inline VkResult
  appendImageSub(nvvk::Image& image, const VkOffset3D& offset,
                 const VkExtent3D& extent,
                 const VkImageSubresourceLayers& subresource, std::span<T> data,
                 VkImageLayout newLayout = VK_IMAGE_LAYOUT_UNDEFINED,
                 const SemaphoreState& semaphoreState = {})
  {
    return appendImageSub(image, offset, extent, subresource, data.size_bytes(),
                          data.data(), newLayout, semaphoreState);
  }
  ...
  // records pending operations (copy & relevant layout transitions) into the
  // command buffer and then resets the internal state for appended.
