...
// Makes stb_image_write take UTF-8 strings instead of the default code page as
// input on Windows
...
VkResult nvvk::imageToLinear(VkCommandBuffer cmd, VkDevice device,
  ...
  // We want host visible and coherent memory to be able to map it and write to
  // it directly
  ...
  nvvk::cmdImageMemoryBarrier(cmd, {srcImage, VK_IMAGE_LAYOUT_GENERAL,
                                    VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL});
  nvvk::cmdImageMemoryBarrier(cmd, {dstImage, VK_IMAGE_LAYOUT_UNDEFINED,
                                    VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL});

  // Do the actual blit from the swapchain image to our host visible destination
  // image The Blit allow to convert the image from VK_FORMAT_B8G8R8A8_UNORM to
  // VK_FORMAT_R8G8B8A8_UNORM automatically
  ...
  vkCmdBlitImage(cmd, srcImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, dstImage,
...
void nvvk::saveImageToFile(VkDevice device, VkImage dstImage,
  ...
  if (core::extensionMatches(filename, ".png")) {
    ...
    filenameUtf8 = core::utf8FromPath(path);
    std::vector<uint8_t> pixels8(size.width * size.height * 4);
    copyImageData(pixels8, 4 * sizeof(uint8_t));
    stbi_write_png(filenameUtf8.c_str(), size.width, size.height, 4,
                   pixels8.data(), size.width * 4);
