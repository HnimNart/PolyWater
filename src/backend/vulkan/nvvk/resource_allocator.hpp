...
//-----------------------------------------------------------------
// Helper classes for allocating Vulkan resources (buffers, images) using VMA
// (Vulkan Memory Allocator).
//
// Contains two main classes:
// - ResourceAllocator: Basic allocation of device-local resources with
// automatic memory management
//   and simplified buffer/image creation. Handles alignment, memory types and
//   resource destruction.
//
// - ResourceAllocatorExport: Extended version that adds support for external
// memory operations,
//   allowing resources to be shared across APIs (Vulkan<->CUDA, Vulkan<->D3D,
//   etc). Includes proper memory flags and handle types for cross-API interop.
//
// Usage:
...
//------------------------------------------------------------------------------------------------------------
//
// Vulkan Memory Allocator (VMA) is a library that helps to manage memory in
...
public:
  ...
  //      ----
  //        + VMA_ALLOCATION_CREATE_MAPPED_BIT // Automatically maps the buffer
  //        upon creation
  //        + VMA_ALLOCATION_CREATE_HOST_ACCESS_SEQUENTIAL_WRITE_BIT // If the
  ...
  // if no fence is provided, a vkQueueWaitIdle will be performed after the
  // binding operation
...
private:
  // Adds "nvvkAllocID: <id>" name to the vma allocation, useful for vma's leak
  // detection (see comments around m_leakID)
...
// This allocator extends ResourceAllocator to enable interoperability with
// external APIs like CUDA and OpenGL. It adds memory export flags during
