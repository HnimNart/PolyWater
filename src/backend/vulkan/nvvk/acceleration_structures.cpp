...
void AccelerationStructureBuildData::addGeometry(
...
void AccelerationStructureBuildData::addGeometry(
    const AccelerationStructureGeometryInfo &asGeom) {
  asGeometry.push_back(asGeom.geometry);
  asBuildRangeInfo.push_back(asGeom.rangeInfo);
}

VkAccelerationStructureBuildSizesInfoKHR
AccelerationStructureBuildData::finalizeGeometry(
    VkDevice device, VkBuildAccelerationStructureFlagsKHR flags) {
...
AccelerationStructureBuildData::makeCreateInfo() const {
  assert(asType != VK_ACCELERATION_STRUCTURE_TYPE_MAX_ENUM_KHR &&
         "Acceleration Structure Type not set");
...
AccelerationStructureBuildData::makeInstanceGeometry(
    size_t numInstances, VkDeviceAddress instanceBufferAddr) {
  assert(asType == VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_KHR &&
         "Instance geometry can only be used with TLAS");
...
void AccelerationStructureBuildData::cmdBuildAccelerationStructure(
    VkCommandBuffer cmd, VkAccelerationStructureKHR accelerationStructure,
    VkDeviceAddress scratchAddress) {
  assert(asGeometry.size() == asBuildRangeInfo.size() &&
         "asGeometry.size() != asBuildRangeInfo.size()");
  assert(accelerationStructure != VK_NULL_HANDLE &&
         "Acceleration Structure not created, first call "
         "createAccelerationStructure");

  const VkAccelerationStructureBuildRangeInfoKHR *rangeInfo =
      asBuildRangeInfo.data();
  ...
  buildInfo.pGeometries =
      asGeometry.data(); // In case the structure was copied, we need to update
                         // the pointer
  buildInfo.geometryCount = static_cast<uint32_t>(asGeometry.size());
...
void AccelerationStructureBuildData::cmdUpdateAccelerationStructure(
    VkCommandBuffer cmd, VkAccelerationStructureKHR accelerationStructure,
    VkDeviceAddress scratchAddress) {
  assert(asGeometry.size() == asBuildRangeInfo.size() &&
         "asGeometry.size() != asBuildRangeInfo.size()");
...
VkResult AccelerationStructureBuilder::cmdCreateBlas(
    VkCommandBuffer cmd,
  ...
  VkDeviceSize scratchAddressEnd =
      scratchAddress + scratchSize; // The end address of the scratch buffer.
  VkDeviceSize budgetUsed =
      0; // Initialize the total budget used in this function call

  // Process each BLAS in the data vector while staying under the memory budget.
  while (m_currentBlasIdx < blasBuildData.size() &&
         budgetUsed < hintMaxBudget) {
    // Build acceleration structures and accumulate the total memory used.
    NVVK_FAIL_RETURN(cmdBuildAccelerationStructures(
        cmd, budgetUsed, blasBuildData, blasAccel, scratchAddress,
  ...
  VkResult status =
      (m_currentBlasIdx < blasBuildData.size()) ? VK_INCOMPLETE : VK_SUCCESS;

  return status;
}

// Initializes a query pool for recording acceleration structure properties if
// necessary. This function ensures a query pool is available if any BLAS in the
// build data is flagged for compaction.
VkResult AccelerationStructureBuilder::initializeQueryPoolIfNeeded(
    VkCommandBuffer cmd,
  ...
  if (needsCompaction) {
    ...
    // Calculate how many BLAS we can potentially build in this batch based on
    // budget
    uint32_t maxBatchSize =
        static_cast<uint32_t>(blasBuildData.size() - m_currentBlasIdx);
...
// Builds multiple Bottom-Level Acceleration Structures (BLAS) for a Vulkan ray
// tracing pipeline. This function manages memory budgets and submits the
// necessary commands to the specified command buffer.
//
// Parameters:
//   cmd               - Command buffer where acceleration structure commands
//   are recorded. budgetUsed        - Running value of the budget used.
//   blasBuildData     - Vector of data structures containing the geometry and
//   other build-related information for each BLAS. blasAccel         - Vector
//   where the function will store the created acceleration structures.
//   scratchAddress    - The starting scratchAddress
//   scratchAddressEnd - The end address for the available scratch space
//   hintMaxBudget     - A hint for the maximum budget allowed for building
//   acceleration structures. currentQueryIdx   - Reference to the current index
//   for queries, updated during execution.
//
// Returns:
//   The total device size used for building the acceleration structures during
//   this function call.
VkResult AccelerationStructureBuilder::cmdBuildAccelerationStructures(
    VkCommandBuffer cmd, VkDeviceSize &budgetUsed,
    std::span<AccelerationStructureBuildData> &blasBuildData,
    std::span<AccelerationStructure> &blasAccel, VkDeviceAddress scratchAddress,
    VkDeviceAddress scratchAddressEnd, VkDeviceSize hintMaxBudget,
    uint32_t &currentQueryIdx, VkQueryPool queryPool) {
  ...
  // available
  while (scratchAddress < scratchAddressEnd && budgetUsed < hintMaxBudget &&
    ...
    NVVK_FAIL_RETURN(
        m_alloc->createAcceleration(blasAccel[m_currentBlasIdx], createInfo));
    ...
    VkDeviceSize scratchSize =
        nvutils::align_up(data.sizeInfo.buildScratchSize, m_scratchAlignment);
  ...
  accelerationStructureBarrier(
      cmd, VK_ACCESS_ACCELERATION_STRUCTURE_WRITE_BIT_KHR,
      VK_ACCESS_ACCELERATION_STRUCTURE_READ_BIT_KHR |
          VK_ACCESS_ACCELERATION_STRUCTURE_WRITE_BIT_KHR);
  ...
  if (queryPool != VK_NULL_HANDLE) {
    ...
    vkCmdWriteAccelerationStructuresPropertiesKHR(
        cmd, numQueries, collectedAccel.data(),
...
// Compacts the Bottom-Level Acceleration Structures (BLAS) that have been
// built, reducing their memory footprint. This function uses the results from
// previously performed queries to determine the compacted sizes and then
// creates new, smaller acceleration structures. It also handles copying from
// the original to the compacted structures.
//
// Notes:
//   It assumes that a query has been performed earlier to determine the
//   possible compacted sizes of the acceleration structures. This function may
//   need to be called multiple times to compact all BLAS in the batch. Check
//   for the return value VK_INCOMPLETE or VK_SUCCESS.
//
VkResult AccelerationStructureBuilder::cmdCompactBlas(
    VkCommandBuffer cmd,
    std::span<AccelerationStructureBuildData> &blasBuildData,
    std::span<AccelerationStructure> &blasAccel) {
  if (m_batches.empty()) {
  ...
  m_batches.pop();
  {
    if (batch.queryPool == VK_NULL_HANDLE) {
      return m_batches.empty()
                 ? VK_SUCCESS
                 : VK_INCOMPLETE; // Skip batches that don't need compaction
    }
    ...
    VkResult result = vkGetQueryPoolResults(
        m_device, batch.queryPool, batch.startIdx, batchSize,
        batchSize * sizeof(VkDeviceSize), compactSizes.data(),
        sizeof(VkDeviceSize),
    ...
    for (uint32_t i = 0; i < batchSize; i++) {
      ...
      if (compactSize > 0) {
        ...
        m_stats.totalOriginalSize +=
            blasBuildData[blasIdx].sizeInfo.accelerationStructureSize;
        blasBuildData[blasIdx].sizeInfo.accelerationStructureSize = compactSize;
        m_cleanupBlasAccel.push_back(
            blasAccel[blasIdx]); // Schedule old BLAS for cleanup.
        ...
        NVVK_FAIL_RETURN(
            m_alloc->createAcceleration(blasAccel[blasIdx], asCreateInfo));
        ...
        copyInfo.src =
            blasBuildData[blasIdx].buildInfo.dstAccelerationStructure;
...
// Find a scratch size that is within the budget, or at least as big as the
// largest required scratch.
VkDeviceSize AccelerationStructureBuilder::getScratchSize(
  ...
  for (auto &buildInfo : buildData) {
    VkDeviceSize alignedSize = nvutils::align_up(
        buildInfo.sizeInfo.buildScratchSize, m_scratchAlignment);
...
// Generates a formatted string summarizing the statistics of BLAS compaction
// results. The output includes the original and compacted sizes in megabytes
// (MB), the amount of memory saved, and the percentage reduction in size. This
// method is intended to provide a quick, human-readable summary of the
// compaction efficiency.
//
// Returns:
//   A string containing the formatted summary of the BLAS compaction
//   statistics.
std::string AccelerationStructureBuilder::Stats::toString() const {
  ...
  const float fractionSmaller =
      (totalOriginalSize == 0)
          ? 0.0f
          : savedSize / static_cast<float>(totalOriginalSize);

  const std::string output = fmt::format(
      "BLAS Compaction: {} bytes -> {} bytes ({} bytes saved, {:.2f}% smaller)",
...
// Returns the maximum scratch buffer size needed for building all provided
// acceleration structures. This function iterates through a vector of
// AccelerationStructureBuildData, comparing the scratch size required for each
// structure and returns the largest value found.
//
// Returns:
//   The maximum scratch size needed as a VkDeviceSize.
VkDeviceSize getMaxScratchSize(
    const std::vector<AccelerationStructureBuildData> &asBuildData) {
...
void AccelerationStructureHelper::init(
    ResourceAllocator *alloc, StagingUploader *uploader, QueueInfo queueInfo,
  ...
  m_transientPool =
      createTransientCommandPool(alloc->getDevice(), queueInfo.familyIndex);
...
void AccelerationStructureHelper::blasSubmitBuildAndWait(
    const std::vector<AccelerationStructureGeometryInfo> &asGeoInfoSet,
    VkBuildAccelerationStructureFlagsKHR buildFlags) {
  VkDevice device = m_alloc->getDevice();

  assert(blasSet.empty() &&
         "we must not invoke build if already built. use deinit before.");
  ...
  // Find the most optimal size for our scratch buffer, and get the addresses of
  // the scratch buffers to allow a maximum of BLAS to be built in parallel,
  // within the budget
  AccelerationStructureBuilder blasBuilder;
  blasBuilder.init(m_alloc);
  VkDeviceSize hintScratchBudget =
      m_blasScratchBudget; // Limiting the size of the scratch buffer to 2MB
  VkDeviceSize scratchSize =
      blasBuilder.getScratchSize(hintScratchBudget, blasBuildData);

  NVVK_CHECK(m_alloc->createBuffer(
      blasScratchBuffer, scratchSize,
  ...
  VkDeviceSize hintBuildBudget =
      m_blasAccelerationStructureBudget; // Limiting the size of the scratch
                                         // buffer to 2MB
  bool finished = false;
  ...
  do {
    {
      ...
      VkResult result = blasBuilder.cmdCreateBlas(
          cmd, buildDataSpan, blasSpan, blasScratchBuffer.address,
      ...
      NVVK_CHECK(endSingleTimeCommands(cmd, device, m_transientPool,
                                       m_queueInfo.queue));
    }
...
void AccelerationStructureHelper::tlasSubmitBuildAndWait(
    const std::vector<VkAccelerationStructureInstanceKHR> &tlasInstances,
    VkBuildAccelerationStructureFlagsKHR buildFlags) {
  VkDevice device = m_alloc->getDevice();

  // we must not invoke build if already built. use update.
  assert((tlasInstancesBuffer.buffer == VK_NULL_HANDLE) &&
         "Do not invoke build if already built. build with "
         "VK_BUILD_ACCELERATION_STRUCTURE_ALLOW_UPDATE_BIT_KHR, then use "
         "tlasUpdate");
  ...
  NVVK_CHECK(m_uploader->appendBuffer(
      tlasInstancesBuffer, 0,
      std::span<VkAccelerationStructureInstanceKHR const>(tlasInstances)));
  NVVK_DBG_NAME(tlasInstancesBuffer.buffer);
  m_uploader->cmdUploadAppended(cmd);
  // Barrier to ensure transfer write completes before acceleration structure
  // build. VK_ACCESS_2_SHADER_READ_BIT is required because the acceleration
  // structure build operation reads the instance data from the buffer (not just
  // writes to the AS). Without this flag, validation layers report
  // READ_AFTER_WRITE hazards.
  ...
  AccelerationStructureGeometryInfo geometryInfo =
      tlasBuildData.makeInstanceGeometry(tlasInstances.size(),
                                         tlasInstancesBuffer.address);
  tlasBuildData.addGeometry(geometryInfo);

  // Get the size of the TLAS
  auto sizeInfo =
      tlasBuildData.finalizeGeometry(m_alloc->getDevice(), buildFlags);

  // Create the scratch buffer
  NVVK_CHECK(m_alloc->createBuffer(
      tlasScratchBuffer, sizeInfo.buildScratchSize,
      VK_BUFFER_USAGE_2_SHADER_DEVICE_ADDRESS_BIT |
          VK_BUFFER_USAGE_2_STORAGE_BUFFER_BIT |
          VK_BUFFER_USAGE_2_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR,
      VMA_MEMORY_USAGE_AUTO, {},
  ...
  tlasBuildData.cmdBuildAccelerationStructure(cmd, tlas.accel,
                                              tlasScratchBuffer.address);

  tlasSize = tlasInstances.size();

  NVVK_CHECK(
      endSingleTimeCommands(cmd, device, m_transientPool, m_queueInfo.queue));

  m_uploader->releaseStaging();
}

void AccelerationStructureHelper::tlasSubmitUpdateAndWait(
    const std::vector<VkAccelerationStructureInstanceKHR> &tlasInstances) {
  VkDevice device = m_alloc->getDevice();
  ...
  if (tlasScratchBuffer.buffer == VK_NULL_HANDLE) {
    NVVK_CHECK(m_alloc->createBuffer(
        tlasScratchBuffer, tlasBuildData.sizeInfo.buildScratchSize,
        VK_BUFFER_USAGE_2_SHADER_DEVICE_ADDRESS_BIT |
            VK_BUFFER_USAGE_2_STORAGE_BUFFER_BIT,
        VMA_MEMORY_USAGE_AUTO, {},
        m_accelStructProps.minAccelerationStructureScratchOffsetAlignment));
  ...
  if (sizeChanged) {
    ...
    tlasBuildData.cmdUpdateAccelerationStructure(cmd, tlas.accel,
                                                 tlasScratchBuffer.address);
  }
