...
void nvvk::cmdGenerateMipmaps(
    VkCommandBuffer cmd, VkImage image, const VkExtent2D& size,
    uint32_t levelCount, uint32_t layerCount /*= 1*/,
    VkImageLayout currentLayout /*= VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL*/)
{
  ...
  for (uint32_t i = 1; i < levelCount; i++)
  {
    ...
    blitRegion.dstOffsets[1] = {mipWidth > 1 ? mipWidth / 2 : 1,
    ...
    // Next mip-level
    {
      // Transition the current mip-level into a eTransferSrcOptimal layout, to
  ...
  // Transition all mip-levels (now in VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL)
  // back to currentLayout
  std::tuple<VkPipelineStageFlags2, VkAccessFlags2> transition =
      nvvk::inferPipelineStageAccessTuple(currentLayout);
