...
/*
 *  HDR sampling is loading an HDR image and creating an acceleration structure
...
//--------------------------------------------------------------------------------------------------
// Loading the HDR environment texture (HDR) and create the important accel
// structure
//
// Note: enableMipmaps will create a mipmap chain for the environment texture,
...
void HdrIbl::loadEnvironment(VkCommandBuffer cmd,
  ...
  if (m_valid) {
    ...
    {
      core::ScopedTimer st("Generating Acceleration structure");
      {
        // Creating the importance sampling for the HDR and storing the info in
        // the m_accelImpSmpl buffer
        std::vector<shaderio::EnvAccel> envAccel = createEnvironmentAccel(
            pixels, imgSize.width, imgSize.height, m_average, m_integral);

        NVVK_CHECK(m_alloc->createBuffer(m_accelImpSmpl,
                                         std::span(envAccel).size_bytes(),
                                         VK_BUFFER_USAGE_2_STORAGE_BUFFER_BIT));
        NVVK_CHECK(
            staging.appendBuffer(m_accelImpSmpl, 0, std::span(envAccel)));
        NVVK_DBG_NAME(m_accelImpSmpl.buffer);

        NVVK_CHECK(m_alloc->createImage(m_texHdr, imageInfo,
                                        DEFAULT_VkImageViewCreateInfo));
        NVVK_CHECK(staging.appendImage(
            m_texHdr, std::span(pixels, buffer_size / sizeof(float)),
    ...
    NVVK_CHECK(m_alloc->createImage(m_texHdr, imageInfo,
                                    DEFAULT_VkImageViewCreateInfo));
    NVVK_CHECK(staging.appendImage(m_texHdr, std::span(color),
                                   VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL));
    NVVK_DBG_NAME(m_texHdr.image);

    NVVK_CHECK(m_alloc->createBuffer(m_accelImpSmpl,
                                     std::span(color).size_bytes(),
                                     VK_BUFFER_USAGE_2_STORAGE_BUFFER_BIT));
    NVVK_DBG_NAME(m_accelImpSmpl.buffer);
  }

  // Sampler for the HDR
  // The map is parameterized with the U axis corresponding to the azimuthal
  // angle, and V to the polar angle Therefore, in U the sampler will use
  // VK_SAMPLER_ADDRESS_MODE_REPEAT (default), but V needs to use CLAMP_TO_EDGE
  // to avoid having light leaking from one pole to another.
...
void HdrIbl::createDescriptorSetLayout() {
  ...
  writeContainer.append(
      bindings.getWriteSet(shaderio::EnvBindings::eHdr, m_descPack.getSet(0)),
      m_texHdr);
  writeContainer.append(bindings.getWriteSet(shaderio::EnvBindings::eImpSamples,
                                             m_descPack.getSet(0)),
                        m_accelImpSmpl);
  vkUpdateDescriptorSets(m_device, static_cast<uint32_t>(writeContainer.size()),
...
createEnvironmentAccel(float *&pixels, const uint32_t &width,
  ...
  // subtended by the texel, and store the weighted luminance in
  ...
  // each smaller radiance texel will be assigned an "alias" with higher emitted
