...
public:
  ...
  //
  // `bindingFlags` will be passed to
  // `VkDescriptorSetLayoutBindingFlagsCreateInfo`; it can be used, for
  // instance, to let a descriptor be updated after it's bound.
  ...
  void addBinding(const VkDescriptorSetLayoutBinding& layoutBinding,
                  VkDescriptorBindingFlags bindingFlags = 0);
  void addBindings(std::span<const VkDescriptorSetLayoutBinding> layoutBindings,
                   VkDescriptorBindingFlags bindingFlags = 0);
  void addBindings(
  ...
  //
  // If no entry exists for the given `binding`, returns a
  // `VkWriteDescriptorSet` with .descriptorType set to
  ...
  // Once the bindings have been added, this generates the descriptor layout
  // corresponding to the bound resources.
  VkResult
  ...
  // If `totalVariableCount` is non zero, it will be used to override the total
  // requirement for the variable binding. Otherwise the regular
  // `binding.descriptorCount * numSets` is used.
  void appendPoolSizes(std::vector<VkDescriptorPoolSize>& poolSizes,
                       uint32_t numSets = 1,
                       uint32_t totalVariableCount = 0) const;
  ...
  calculatePoolSizes(uint32_t numSets = 1,
                     uint32_t totalVariableCount = 0) const;
...
private:
  ...
  // Map from `VkDescriptorSetLayoutBinding::binding` to an index in the above
  // arrays. Vulkan recommends using as compact a maximum binding number as
  // possible, so a linear array should be OK.
...
public:
  ...
  //
  // If `totalVariableCount` is non zero, it will be used to override the total
  // requirement for the variable binding, and `descriptorVariableCounts` must
  // be non null and the length of `numSets`. Otherwise the regular
  // `binding.descriptorCount * numSets` is used.
  VkResult init(const DescriptorBindings& bindings, VkDevice device,
                uint32_t numSets = 1,
  ...
  const VkDescriptorSet* getSetPtr(uint32_t setIndex = 0) const
  {
    return &m_sets[setIndex];
  }

  // Wrapper to get a `VkWriteDescriptorSet` for a descriptor set stored in
  // `sets` if it's not empty. Empty `sets` usage is legal in the push
  // descriptor use-case. see `DescriptorBindings::getWriteSet` for more details
  VkWriteDescriptorSet makeWrite(uint32_t binding, uint32_t setIndex = 0,
                                 uint32_t dstArrayElement = ~0,
                                 uint32_t descriptorCount = 1) const
  {
    return m_bindings.getWriteSet(
        binding, m_sets.empty() ? VK_NULL_HANDLE : m_sets[setIndex],
...
inline VkResult createPipelineLayout(
    VkDevice device, VkPipelineLayout* pPipelineLayout,
    ...
    std::initializer_list<const VkPushConstantRange> pushConstantRanges = {})
{
  return createPipelineLayout(
      device, pPipelineLayout,
      std::span<const VkDescriptorSetLayout>(layouts.begin(), layouts.size()),
      std::span<const VkPushConstantRange>(pushConstantRanges.begin(),
...
public:
  ...
  void append(const VkWriteDescriptorSet& writeSet, const nvvk::Buffer& buffer,
              VkDeviceSize offset = 0, VkDeviceSize range = VK_WHOLE_SIZE);
  void append(const VkWriteDescriptorSet& writeSet,
              const nvvk::AccelerationStructure& accel);
  void append(const VkWriteDescriptorSet& writeSet, const nvvk::Image& image);

  void append(const VkWriteDescriptorSet& writeSet, VkBuffer buffer,
              VkDeviceSize offset = 0, VkDeviceSize range = VK_WHOLE_SIZE);
  void append(const VkWriteDescriptorSet& writeSet, VkBufferView bufferView);
  void append(const VkWriteDescriptorSet& writeSet,
              const VkDescriptorBufferInfo& bufferInfo);
  void append(const VkWriteDescriptorSet& writeSet, VkImageView imageView,
  ...
  void append(const VkWriteDescriptorSet& writeSet,
              const nvvk::Buffer* buffers);  // offset 0 and VK_WHOLE_SIZE
  void append(const VkWriteDescriptorSet& writeSet,
...
private:
  ...
  static_assert(sizeof(VkDescriptorBufferInfo) ==
