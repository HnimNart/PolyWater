...
static_assert(VK_HEADER_VERSION >= 304,
              "nvcontext.hpp requires at least Vulkan SDK 1.4.304. "
...
struct ExtensionInfo
{
  const char* extensionName =
      nullptr;  // Name of the extension ex. VK_KHR_SWAPCHAIN_EXTENSION_NAME
  void* feature =
      nullptr;  // [optional] Pointer to the feature structure for the extension
  bool required = true;  // [optional] If the extension is required
  uint32_t specVersion =
      0;  // [optional] Spec version of the extension, this version or higher
  bool exactSpecVersion =
      false;  // [optional] If true, the spec version must match exactly
};

// Specific to the creation of Vulkan context
// instanceExtensions   : Instance extensions:
// VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME deviceExtensions     :
// Device extensions: {{VK_KHR_SWAPCHAIN_EXTENSION_NAME},
// {VK_KHR_ACCELERATION_STRUCTURE_EXTENSION_NAME, &accelFeature}, {OTHER}}
// queues               : All desired queues
// instanceCreateInfoExt: Instance create info extension (ex:
// VkLayerSettingsCreateInfoEXT) applicationName      : Application name
// apiVersion           : Vulkan API version
// alloc                : Allocation callbacks
// enableAllFeatures    : If true, pull all capability of `features` from the
// physical device forceGPU             : If != -1, use GPU index, useful to
...
struct ContextInitInfo
{
  ...
  // Return true to allow this physical device to be selected, false to reject
  // it.
  std::optional<std::function<bool(VkInstance, VkPhysicalDevice)>>
      preSelectPhysicalDeviceCallback;
  // [optional] Callback called after device selection but before device
  // creation. Can modify ContextInitInfo to add/remove extensions, change queue
  // config, etc. Return true to proceed with device creation, false to abort.
  std::optional<
      std::function<bool(VkInstance, VkPhysicalDevice, ContextInitInfo&)>>
      postSelectPhysicalDeviceCallback;
};
...
public:
  ...
  {
    return m_queueInfos[index];
  }
  const std::vector<nvvk::QueueInfo>& getQueueInfos() const
  {
    return m_queueInfos;
  }
  ...
  {
    return m_deviceFeatures.features;
  }
  const VkPhysicalDeviceVulkan11Features& getPhysicalDeviceFeatures11() const
  {
    return m_deviceFeatures11;
  }
  const VkPhysicalDeviceVulkan12Features& getPhysicalDeviceFeatures12() const
  {
    return m_deviceFeatures12;
  }
  const VkPhysicalDeviceVulkan13Features& getPhysicalDeviceFeatures13() const
  {
    return m_deviceFeatures13;
  }
  const VkPhysicalDeviceVulkan14Features& getPhysicalDeviceFeatures14() const
  {
    return m_deviceFeatures14;
  }

public:
  // Those functions are used internally to create the Vulkan context, but could
...
private:
  ...
  // Filters available Vulkan extensions based on desired extensions and their
  // specifications.
  bool filterAvailableExtensions(
...
void addSurfaceExtensions(
    std::vector<const char*>& instanceExtensions,
    std::vector<nvvk::ExtensionInfo>* deviceExtensions = nullptr);
