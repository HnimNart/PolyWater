...
void dumpPipelineStats(VkDevice device, VkPipeline pipeline,
                       const std::filesystem::path& fileName)
{
  ...
  vkGetPipelineExecutablePropertiesKHR(device, &pipeInfo, &executableCount,
                                       nullptr);
  props.resize(executableCount,
  ...
  for (uint32_t i = 0; i < executableCount; i++)
  {
    ...
    vkGetPipelineExecutableStatisticsKHR(device, &execInfo, &statsCount,
                                         nullptr);
...
void dumpPipelineInternals(VkDevice device, VkPipeline pipeline,
  ...
  for (uint32_t e = 0; e < executableCount; e++)
  {
    ...
    if (internalCount)
    {
      ...
      vkGetPipelineExecutableInternalRepresentationsKHR(
          device, &execInfo, &internalCount, internals.data());
      ...
      for (uint32_t i = 0; i < internalCount; i++)
      {
        ...
        fileName += std::string(".") + prop.name + "." + std::to_string(e) +
                    "." + internals[i].name + "." + std::to_string(i) + "." +
                    (isText ? "txt" : "bin");
