...
void SubmitInfo::append(const CmdPreSubmitInfo& preSubmit,
                        ...
                        uint32_t signalDeviceIndex)
{
  ...
  waitSemaphoreStates.insert(waitSemaphoreStates.end(),
                             preSubmit.waitSemaphores.cbegin(),
                             preSubmit.waitSemaphores.cend());
  signalSemaphoreStates.insert(signalSemaphoreStates.end(),
                               preSubmit.signalSemapores.cbegin(),
...
VkResult QueueTimeline::submit(SubmitInfo& submitInfo,
                               SemaphoreState& submitState, VkFence fence)
{
  ...
  {
    assert(it.semaphoreState.isFixed() &&
           "waitSemaphoreStates must have been finalized");
  ...
  for (const VkSemaphoreSubmitInfo& it : signalSemaphores)
  {
    assert(it.semaphore != m_timelineSemaphore &&
           "regular signalSemaphores must not use queue's timelineSemaphore, "
           "use signalSemaphoreStates instead");
  }
#endif
  ...
  uint64_t submitTimeLineValue = 0;
  {
    ...
    for (SemaphoreSubmitState& it : submitInfo.signalSemaphoreStates)
    {
      ...
      if (it.semaphoreState.getSemaphore() == m_timelineSemaphore)
      {
        assert(!submitState.isValid() &&
               "must not use submitInfo.signalSemaphoreState using queue "
               "timelineSemaphore more than once");
...
[[maybe_unused]] static void usage_QueueTimeline()
{
  ...
  // basic operations
  {
    ...
    {
      // The main purpose of the `nvvk::QueueTimeline` object is to wrap its
      // timeline semaphore that is incremented with each submit.
      //
      // The `nvvk::SemaphoreState` object can be used to get information about
      // a future submit, in such a way that it can be stored and queried safely
      // at a later time.
      //
      // This can be useful when there is more users of a `nvvk::QueueTimeline`
      // and it isn't as easy to figure out the order of submits done to it (and
      // therefore the corresponding timeline values). The
      // `nvvk::SemaphoreState` acts as a `future` to a pending signal
      // operation.

      // In this basic sample we get a command buffer every frame
      VkCommandBuffer cmd{};

      // Also get a semaphore state, so that we detect the completion of the
      // command buffer. Note at this point in time we haven't submitted the
      // command buffer yet, but this object is safe to use and copy even if
      // someone else were to submit to `queueTimelineA` before above `cmd` is.
      //
      // Until it was signaled, this semaphoreState is `dynamic` with an unknown
      // timeline value.

      nvvk::SemaphoreState semaphoreState =
          queueTimelineA.createDynamicSemaphoreState();

      // let's fake actually caring for the completion of cmd
      if (!semaphoreStateTest.isValid())
      {
        // copy this frame's semaphore state, even if it hasn't been
        // signaled/submitted yet
      ...
      else
      {
        ...
        {
          // do something, knowing that `cmd` from the time we made the
          // `semaphoreStateTest` copy assignment was completed
        }
      }

      // the submits to the `QueueTimeline` object are done through a wrapper
      // struct similar to `VkSubmitInfo2`

      submitInfo.clear();
      submitInfo.append(cmd);

      // don't forget to insert the dynamic semaphore state from this frame to
      // be signaled with the submit.
      submitInfo.signalSemaphoreStates.push_back(
          {std::move(semaphoreState), VK_PIPELINE_STAGE_2_BOTTOM_OF_PIPE_BIT});

      // the submit process returns a fixed semaphore state, as now the timeline
      // value is known.
      nvvk::SemaphoreState semaphoreSubmittedState;
      VkResult result =
          queueTimelineA.submit(submitInfo, semaphoreSubmittedState);
    }
  }

  // more complex scenario implementing a garbage collector for a secondary
  // queue
  {
    ...
    while (true)
    {
      ...
      // We can pass this struct around to have something more portable for
      // other code.
      ...
      {
        // some inner render loop function where we don't want to expose all of
        // application instead it is handed over `preSubmitA` as &
        //
        // it wants to do a submit to B that cmdA should wait for

        VkCommandBuffer cmdB{};
        nvvk::SemaphoreState semaphoreStateB =
            queueTimelineB.createDynamicSemaphoreState();

        {
          // let's say we have some staging data that we need for this operation
          // on queueB

          // first delete old completed garbage
          while (!garbageEntries.empty())
          {
            // we copied this semaphoreState in a previous frame, see a bit
            // later
            ...
            {
              // we pushed in order, so if first isn't completed then others
              // cannot either
              break;
            }
          }
        ...
        preSubmitA.waitSemaphores.push_back(
            {.semaphoreState = std::move(semaphoreStateB)});
      }
