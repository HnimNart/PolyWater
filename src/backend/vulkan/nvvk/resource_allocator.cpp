...
void nvvk::ResourceAllocator::addLeakDetection(VmaAllocation allocation) const
{
  ...
  std::string nvvkAllocID =
      fmt::format("nvvkAllocID: {}", m_allocationCounter++);
    ...
    VkDeviceSize minAlignment, std::span<const uint32_t> queueFamilies) const
{
  ...
  const VkBufferCreateInfo bufferInfo{
      ...
      .size = size,
      .usage = 0,
      .sharingMode = queueFamilies.empty() ? VK_SHARING_MODE_EXCLUSIVE
    ...
    VkDeviceSize minAlignment) const
{
  ...
  else
  {
    ...
    memReqs.memoryRequirements.alignment =
        std::max(minAlignment, memReqs.memoryRequirements.alignment);
    ...
    size_t totalChunkCount =
        (bufferInfo.size + maxChunkSize - 1) / maxChunkSize;
    ...
    if (fullChunkCount != totalChunkCount)
    {
      ...
      memReqs.memoryRequirements.size =
          (memReqs.memoryRequirements.size + pageAlignment - 1) &
          ~(pageAlignment - 1);
    ...
    for (uint32_t i = 0; i < totalChunkCount; i++)
    {
      ...
      sparseBind.size =
          std::min(maxChunkSize, createInfo.size - i * maxChunkSize);
      sparseBind.size =
          (sparseBind.size + pageAlignment - 1) & ~(pageAlignment - 1);
    ...
    std::span<const uint32_t> queueFamilies /*= {}*/) const
{
  ...
  NVVK_FAIL_RETURN(vmaFindMemoryTypeIndexForBufferInfo(
      m_allocator, &bufferInfo, &allocInfo, &memoryTypeIndex));

  allocInfo.usage = VMA_MEMORY_USAGE_UNKNOWN;
  allocInfo.memoryTypeBits = 1 << memoryTypeIndex;

  return createLargeBuffer(largeBuffer, bufferInfo, allocInfo,
    ...
    const VmaAllocationCreateInfo& vmaInfo) const
{
  ...
  NVVK_FAIL_RETURN(vkCreateImageView(m_device, &viewInfo, nullptr,
                                     &image.descriptor.imageView));
    ...
    std::span<const uint32_t> queueFamilies) const
{
...

  {
    ...
    resultAccel.address =
        vkGetAccelerationStructureDeviceAddressKHR(m_device, &info);
  }
    ...
    VkDeviceSize maxChunkSize) const
{
  ...
  return createLargeAcceleration(accel, accInfo, allocInfo, sparseBindingQueue,
                                 sparseBindingFence, maxChunkSize);
}
    ...
    VkDeviceSize minAlignment, std::span<const uint32_t> queueFamilies)
{
  ...
  NVVK_FAIL_RETURN(
      getAllocInfo(allocCreateInfo, flags, memoryUsage, bufferInfo));
    ...
    const VkImageViewCreateInfo& imageViewInfo)
{
  ...
  NVVK_FAIL_RETURN(getAllocInfo(
      allocInfo, 0, VMA_MEMORY_USAGE_AUTO_PREFER_DEVICE, imageInfoCpy));
    ...
    VmaMemoryUsage usage, const VkBufferCreateInfo& bufferInfo)
{
  ...
  // Populate the flags and usage before vmaFindMemoryTypeIndexForBufferInfo
  // since it uses them to query memTypeIndex.
  allocCreateInfo.flags = flags;
  allocCreateInfo.usage = usage;

  NVVK_FAIL_RETURN(vmaFindMemoryTypeIndexForBufferInfo(
      *this, &bufferInfo, &allocCreateInfo, &memTypeIndex));
    ...
    VmaMemoryUsage usage, const VkImageCreateInfo& imageInfo)
{
  ...
  // Populate the flags and usage before vmaFindMemoryTypeIndexForImageInfo
  // since it uses them to query memTypeIndex.
  allocCreateInfo.flags = flags;
  allocCreateInfo.usage = usage;

  NVVK_FAIL_RETURN(vmaFindMemoryTypeIndexForImageInfo(
      *this, &imageInfo, &allocCreateInfo, &memTypeIndex));
  NVVK_FAIL_RETURN(getPool(memTypeIndex, allocCreateInfo.pool));
