...
StagingUploader::operator=(StagingUploader&& other) noexcept
{
  if (this != &other)
  {
    assert(m_resourceAllocator == nullptr &&
           "deinit not called prior move assignment");
...
void StagingUploader::init(ResourceAllocator* resourceAllocator,
                           bool enableLayoutBarriers)
{
  assert(m_resourceAllocator == nullptr);
...
void StagingUploader::deinit()
{
  ...
  {
    releaseStaging(true);
    assert(m_stagingResources.empty() &&
           m_stagingResourcesSize ==
               0);  // must have released all staged uploads
  }
...
VkResult
StagingUploader::acquireStagingSpace(BufferRange& stagingSpace, size_t dataSize,
                                     const void* data,
                                     const SemaphoreState& semaphoreState)
{
  ...
  // VMA_MEMORY_USAGE_AUTO_PREFER_HOST staging memory is meant to not cost
  // additional device memory
  //
  // VMA_ALLOCATION_CREATE_HOST_ACCESS_SEQUENTIAL_WRITE_BIT staging memory is
  // filled sequentially VMA_ALLOCATION_CREATE_MAPPED_BIT staging memory is
  // filled through pointer access
  //
  // VK_MEMORY_PROPERTY_HOST_COHERENT_BIT we want to avoid having to call
  // "vkFlushMappedMemoryRanges"
  //
  // As of writing VMA doesn't have a simple usage that guarantees
  // VK_MEMORY_PROPERTY_HOST_COHERENT_BIT (only the deprecated
  // VMA_MEMORY_USAGE_CPU_ONLY did)
  ...
  NVVK_FAIL_RETURN(m_resourceAllocator->createBuffer(stagingResource.buffer,
...
{
  return m_batch.copyBufferImageInfos.empty() &&
         m_batch.copyBufferInfos.empty();
}
                                       ...
                                       const SemaphoreState& semaphoreState)
{
  ...
  else
  {
    ...
    NVVK_FAIL_RETURN(
        acquireStagingSpace(stagingSpace, dataSize, data, semaphoreState));
...
VkResult
StagingUploader::appendBufferRange(const nvvk::BufferRange& bufferRange,
                                   const void* data,
                                   const SemaphoreState& semaphoreState)
{
  // allow empty without throwing error
  ...
  {
    BufferRange stagingSpace;
    NVVK_FAIL_RETURN(acquireStagingSpace(stagingSpace, bufferRange.range, data,
                                         semaphoreState));
...
VkResult StagingUploader::appendBufferMapping(
    const nvvk::Buffer& buffer, VkDeviceSize bufferOffset,
    VkDeviceSize dataSize, void*& uploadMapping,
    const SemaphoreState& semaphoreState)
{
  ...
  else
  {
    BufferRange stagingSpace;
    NVVK_FAIL_RETURN(
        acquireStagingSpace(stagingSpace, dataSize, nullptr, semaphoreState));
...
VkResult
StagingUploader::appendBufferRangeMapping(const nvvk::BufferRange& bufferRange,
                                          void*& uploadMapping,
                                          const SemaphoreState& semaphoreState)
{
  uploadMapping = nullptr;
  ...
  {
    BufferRange stagingSpace;
    NVVK_FAIL_RETURN(acquireStagingSpace(stagingSpace, bufferRange.range,
                                         nullptr, semaphoreState));
...
VkResult StagingUploader::appendImage(nvvk::Image& image, size_t dataSize,
                                      const void* data, VkImageLayout newLayout,
                                      const SemaphoreState& semaphoreState)
{
  BufferRange stagingSpace;
  NVVK_FAIL_RETURN(
  ...
  if (m_enableLayoutBarriers &&
      (!layoutAllowsCopy || newLayout != VK_IMAGE_LAYOUT_UNDEFINED))
  {
    ...
    VkImageMemoryBarrier2 barrier =
        makeImageMemoryBarrier({.image = image.image,
...
VkResult StagingUploader::appendImageSub(
    nvvk::Image& image, const VkOffset3D& offset, const VkExtent3D& extent,
    const VkImageSubresourceLayers& subresource, size_t dataSize,
    const void* data, VkImageLayout newLayout /*= VK_IMAGE_LAYOUT_UNDEFINED*/,
    const SemaphoreState& semaphoreState /*= {}*/)
{
  BufferRange stagingSpace;
  NVVK_FAIL_RETURN(
...
bool StagingUploader::checkAppendedSize(size_t limitInBytes,
                                        size_t addedSize) const
{
  return m_batch.stagingSize &&
         (m_batch.stagingSize + addedSize) > limitInBytes;
}
...
void StagingUploader::cmdUploadAppended(VkCommandBuffer cmd)
{
  ...
  {
    m_batch.copyBufferImageInfos[i].pRegions =
        &m_batch.copyBufferImageRegions[i];
...
void StagingUploader::releaseStaging(bool forceAll)
{
  ...
  for (size_t readIdx = 0; readIdx < m_stagingResources.size(); readIdx++)
  {
    ...
    bool canRelease = forceAll || (!stagingResource.semaphoreState.isValid()) ||
                      stagingResource.semaphoreState.testSignaled(device);
...
[[maybe_unused]] static void usage_StagingUploader()
{
  ...
  // simple example, relying on device wait idle, not using SemaphoreState
  {
    ...
    // we prefer device memory and set a few bits that allow device-mappable
    // memory to be used, but doesn't enforce it
    resourceAllocator.createBuffer(
        buffer, 256, VK_BUFFER_USAGE_2_STORAGE_BUFFER_BIT,
    ...
    // The stagingUploader will detect if the buffer was mappable directly then
    // copy there directly, otherwise copy through a temporary staging buffer.
    //
    // Note we consume the provided pointers directly here, but underlying
    // vulkan commands are "appended" and need to be executed later.
    stagingUploader.appendBuffer(buffer, offset, dataSize, data.data());

    // Execute the upload of all previously appended data to the GPU (if
    // necessary)
  ...
  // batched,  relying on device wait idle for release, not using SemaphoreState
  {
    ...
    for (size_t i = 0; i < myImageTextures.size(); i++)
    {
      ...
      // this will handle the transition from the current
      // `myImageTextures[i].descriptor.imageLayout` to
      // `VK_IMAGE_LAYOUT_READ_ONLY_OPTIMAL` as well as the intermediate
      // transition to `VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL`
      result = stagingUploader.appendImage(myImageTextures[i], myImageDatas[i],
...

  {
    ...
    VkResult result =
        resourceAllocator.createBuffer(myBuffer, std::span(myData).size_bytes(),
    ...
    while (true)
    {
      // release staging resources from past frames based on their
      // SemaphoreState
      stagingUploader.releaseStaging();
      ...
      if (true)
      {
        ...
        result = stagingUploader.appendBuffer(myBuffer, 0, std::span(myData),
                                              semaphoreState);
      }
      else
      {
        ...
        result = stagingUploader.appendBufferMapping(
            myBuffer, 0, std::span(myData).size_bytes(), mappingPointer,
            semaphoreState);
