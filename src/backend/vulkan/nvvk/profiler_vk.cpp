                            ...
                            int queueFamilyIndex, bool useLabels)
{
  ...
  m_useLabels = useLabels && (vkCmdBeginDebugUtilsLabelEXT != nullptr) &&
                                  ...
                                  uint32_t requiredSize)
{
  ...
  uint32_t newCount = (requiredSize + PoolContainer::POOL_QUERY_COUNT - 1) /
                      PoolContainer::POOL_QUERY_COUNT;
  ...
  for (uint32_t i = oldCount; i < newCount; i++)
  {
    ...
    NVVK_CHECK(vkCreateQueryPool(m_device, &createInfo, nullptr,
                                 &container.queryPools[i]));
...
[[maybe_unused]] static void usage_ProfilerGpuTimer()
{
  ...
  gpuTimer.init(profilerTimeline, device, physicalDevice, queueFamilyIndex,
                true);
  ...
  /* while(!glfwWindowShouldClose()) */
  {
    ...
    std::vector<core::ProfilerTimeline::Snapshot> frameSnapshots;
    std::vector<core::ProfilerTimeline::Snapshot> asyncSnapshots;
