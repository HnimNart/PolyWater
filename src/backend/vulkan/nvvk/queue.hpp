...
struct CmdPreSubmitInfo
{
  ...
  // contains the SemaphoreState that is triggered when this command buffer is
  // submitted. By making a copy of it, one can later test if the command buffer
  // has been completed.
  SemaphoreState semaphoreState;

  // add SemaphoreStates here, that this command buffer must wait on when being
  // submitted, or are required to be signaled as well.
...
public:
  ...
  void append(VkCommandBuffer cmd, SemaphoreState signalSemaphoreState,
...
public:
  ...
  // Create fresh semaphore state for any submission you might want to make at a
  // later time. Can create as many as needed, but must be part of
  // `submitInfo.signalSemaphoreStates`
  SemaphoreState createDynamicSemaphoreState() const;

  // returns semaphore state of submit
  VkResult submit(SubmitInfo& info, SemaphoreState& submitState,
