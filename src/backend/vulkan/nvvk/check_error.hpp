...
public:
  ...
  // and line, calls the callback, and asserts false and always calls
  // exit(EXIT_FAILURE).
  void check(VkResult result, const char* expression, const char* file,
             int line);

  // Same as `check`, but is "recoverable"; prints an error and returns the
  // value.
  VkResult report(VkResult result, const char* expression, const char* file,
                  int line);
...
#define NVVK_CHECK(vkFnc)                                                      \
  {                                                                            \
    ...
    nvvk::CheckError::getInstance().check(checkResult, #vkFnc, __FILE__,       \
                                          __LINE__);                           \
  }
...
#define NVVK_FAIL_RETURN(vkFnc)                                                \
  {                                                                            \
    ...
    if (checkResult < 0)                                                       \
    {                                                                          \
      return nvvk::CheckError::getInstance().report(checkResult, #vkFnc,       \
                                                    __FILE__, __LINE__);       \
    }                                                                          \
  }
...
// }
// ```
#define NVVK_FAIL_REPORT(vkFnc)                                                \
  nvvk::CheckError::getInstance().report(vkFnc, #vkFnc, __FILE__, __LINE__)
