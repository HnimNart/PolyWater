...
private:
  ...
  // the OffsetAllocator's size may be bigger given its internal free space
  // search
  uint32_t size{};

  // original requested alignment
  // This alignment may need to be applied when converting the allocation.offset
  // back to actual byte offset
...
public:
  ...
  static constexpr uint32_t MAX_ALIGNMENT =
      1 << (sizeof(BufferSubAllocation::alignmentMinusOne) * 8);
  static constexpr uint32_t MAX_TOTAL_BLOCKS =
      1 << (sizeof(BufferSubAllocation::block) * 8);
  static constexpr VkDeviceSize DEFAULT_BLOCK_SIZE =
      VkDeviceSize(128) * 1024 * 1024;
  ...
  {
    return m_state.maxAllocationSize;
  }
  ...
  // alignment is legal to be non-power-of-two, but must be divisible by
  // MIN_ALIGNMENT, then the returned offsets will be a multiple of the
  // alignment size must be <= min(4GB,
  // VkPhysicalDeviceVulkan11Properties::maxMemoryAllocationSize)
  VkResult subAllocate(BufferSubAllocation& subAllocation, VkDeviceSize size,
...
protected:
  ...
  VkResult createNewBuffer(nvvk::Buffer& buffer, VkDeviceSize size,
                           uint32_t alignment, uint32_t blockIndex);

  uint32_t acquireBlockIndex();

  struct Block
  {
    // can be null for dedicated block that have only a single big allocation >
    // m_info.blockSize
    std::unique_ptr<OffsetAllocator::Allocator> offsetAllocator;
    // can be null if block was fully deallocated
    nvvk::Buffer buffer;
    // continuation of single linked list of blocks that were deallocated
    // completely
  ...
  struct State
  {
    ...
    // adjusted size as the offset allocator operates in units of
    // m_info.minAlignment
    ...
    // active blocks are blocks that have OffsetAllocators (i.e. not dedicated
