...
{
public:
  // this object does not cover viewport and scissor states, we assume these to
  // be done dynamically all the time use this static function instead

  inline static void cmdSetViewportAndScissor(VkCommandBuffer cmd,
  ...
  // Help binding all shaders the first time. In graphics unused stages must be
  // bound with NULL if not used, they cannot be left without binding. Set
  // `withMesh` to true if the mesh shader extension is active.
  inline static void cmdBindShaders(VkCommandBuffer cmd,
                                    const BindableShaders& shaders,
                                    bool withMesh = false)
  {
    ...
    vkCmdBindShadersEXT(cmd, withMesh ? 7 : 5, shaderType.data(),
                        &shaders.vertex);
  }
  ...
  VkPipelineRasterizationStateCreateInfo rasterizationState{
      ...
      // pNext will be set to `rasterizationLineState` when used in
  ...
  VkPipelineMultisampleStateCreateInfo multisampleState{
      ...
      .minSampleShading = 0,
      .pSampleMask = 0,  // do not use, implicitly set to &sampleMask in
                         // GraphicsPipelineCreator
  ...
  VkPipelineColorBlendStateCreateInfo colorBlendState{
      ...
      .attachmentCount =
          0,              // do not use, implicitly set GraphicsPipelineCreator
  ...
  VkPipelineTessellationStateCreateInfo tessellationState{
      ...
      // pNext will be set to `tessellationDomainOriginState` when used in
      // GraphicsPipelineCreator
  ...
  void
  cmdApplyDynamicStates(VkCommandBuffer cmd,
                        std::span<const VkDynamicState> dynamicStates) const;

  // returns true if not handled
  bool cmdApplyDynamicState(VkCommandBuffer cmd,
                            VkDynamicState dynamicState) const;
};
...
public:
  VkGraphicsPipelineCreateInfo pipelineInfo{
      ...
      // if pStages is set manually, then clearShaders() must be used prior
      // `createGraphicsPipeline` all other pointers are automatically
      // configured.
  };
  ...
  void clearShaders();
  void addShader(VkShaderStageFlagBits stage, const char* pEntryName,
  ...
  VkResult createGraphicsPipeline(VkDevice device, VkPipelineCache cache,
                                  const GraphicsPipelineState& graphicsState,
                                  VkPipeline* pPipeline);

protected:
  void buildPipelineCreateInfo(VkGraphicsPipelineCreateInfo& createInfoTemp,
                               const GraphicsPipelineState& graphicsState);

  std::vector<VkPipelineShaderStageCreateInfo> m_shaderStages;
  std::vector<VkPipelineShaderStageRequiredSubgroupSizeCreateInfo>
      m_shaderStageSubgroupSizes;
  ...
  std::vector<VkVertexInputBindingDivisorDescription>
      m_staticVertexBindingDivisors;
...
inline bool
GraphicsPipelineState::cmdApplyDynamicState(VkCommandBuffer cmd,
                                            VkDynamicState dynamicState) const
{
  switch (dynamicState)
  {
    ...
    case VK_DYNAMIC_STATE_DEPTH_BOUNDS_TEST_ENABLE:
      vkCmdSetDepthBoundsTestEnable(cmd,
    ...
    case VK_DYNAMIC_STATE_RASTERIZER_DISCARD_ENABLE:
      vkCmdSetRasterizerDiscardEnable(
          cmd, rasterizationState.rasterizerDiscardEnable);
      return false;
    ...
    case VK_DYNAMIC_STATE_LINE_STIPPLE_ENABLE_EXT:
      vkCmdSetLineStippleEnableEXT(cmd,
                                   rasterizationLineState.stippledLineEnable);
      return false;
    default:
      return true;
  }
