...
//------------------------------------------------------------------------------------------------------------
/*--
 * Swapchain: The swapchain is responsible for presenting rendered images to the
screen.
 * It consists of multiple images (frames) that are cycled through for rendering
and display.
 * The swapchain is created with a surface and optional vSync setting, with the
 * window size determined during its setup.
 * "Frames in flight" refers to the number of images being processed
concurrently (e.g., double buffering = 2, triple buffering = 3).
 * vSync enabled (FIFO mode) uses double buffering, while disabling vSync
(MAILBOX mode) uses triple buffering.
 *
 * The "current frame" is the frame currently being processed.
 * The "next image index" points to the swapchain image that will be rendered
next, which might differ from the current frame's index.
 * If the window is resized or certain conditions are met, the swapchain needs
to be recreated (`needRebuild` flag).
-*/
class Swapchain
{
public:
  ...
  {
    return m_images[m_frameImageIndex].imageView;
  }
  ...
  // Initialize the swapchain with the provided context and surface, then we can
  ...
  /*--
   * Create the swapchain using the provided context, surface, and vSync option.
  The actual window size is returned.
   * Queries the GPU capabilities, selects the best surface format and present
  mode, and creates the swapchain accordingly.
  -*/
  VkResult initResources(VkExtent2D& outWindowSize, bool vSync = true);

  /*--
   * Recreate the swapchain, typically after a window resize or when it becomes
  invalid.
   * This waits for all rendering to be finished before destroying the old
  swapchain and creating a new one.
  -*/
   ...
   * This function handles synchronization with the previous frame and acquires
  the next image from the swapchain.
   ...
   * The semaphore ensures that the image is presented only after rendering is
  complete.
...
private:
  ...
  {
    VkSemaphore imageAvailableSemaphore{};  // Signals when the image is ready
                                            // for rendering
    VkSemaphore
        renderFinishedSemaphore{};  // Signals when rendering is finished
  };

  // We choose the format that is the most common, and that is supported by* the
  // physical device.
  VkSurfaceFormat2KHR selectSwapSurfaceFormat(
      const std::vector<VkSurfaceFormat2KHR>& availableFormats) const;

  /*--
   * The present mode is chosen based on the vSync option
   * The `preferredVsyncOnMode` is used when vSync is enabled and the mode is
  supported.
   * The `preferredVsyncOffMode` is used when vSync is disabled and the mode is
  supported.
   * Otherwise, from most preferred to least:
   *   1. IMMEDIATE mode, when vSync is disabled (tearing allowed), since it's
  lowest-latency.
   *   2. MAILBOX mode, since it's the lowest-latency mode without tearing. Note
  that frame pacing is needed when vSync is on.
...
private:
  ...
  Swapchain&
  operator=(const Swapchain&) = default;  // Allow copy only for internal use in
                                          // deinit() to restore pristine state
  ...
  std::vector<FrameResources>
      m_frameResources{};  // Synchronization primitives for each frame
  uint32_t m_frameResourceIndex =
      0;  // Current frame index, cycles through frame resources
  uint32_t m_frameImageIndex =
      0;  // Index of the swapchain image we're currently rendering to
  bool m_needRebuild =
      false;  // Flag indicating if the swapchain needs to be rebuilt

  VkPresentModeKHR m_preferredVsyncOffMode =
      VK_PRESENT_MODE_IMMEDIATE_KHR;  // used if available
  VkPresentModeKHR m_preferredVsyncOnMode =
      VK_PRESENT_MODE_FIFO_KHR;  // used if available
