...
// This file defines the DebugUtil class, a singleton utility for managing
// Vulkan debug utilities. It provides functionality to set debug names for
// Vulkan objects and manage debug labels in command buffers.
//
// It is initialized with a Vulkan device using the init() method and currently
// called in the Context class. For the debug functions to work, the
// VK_EXT_DEBUG_UTILS_EXTENSION_NAME must be enabled in the instance extensions.
//
// Usage:
// 1. Use DBG_VK_NAME(obj) macro to set debug names for Vulkan objects.
// 2. Use DBG_VK_SCOPE(cmdBuf) macro to create scoped debug labels in command
// buffers.
//
...
public:
  ...
  void setObjectName(T object, const std::string& name) const
  {
    ...
    if (vkSetDebugUtilsObjectNameEXT != nullptr &&
        objectType != VK_OBJECT_TYPE_UNKNOWN && m_device != VK_NULL_HANDLE)
    {
      VkDebugUtilsObjectNameInfoEXT s{
          VK_STRUCTURE_TYPE_DEBUG_UTILS_OBJECT_NAME_INFO_EXT, nullptr,
          objectType, (uint64_t) object, name.c_str()};
...
#define NVVK_DBG_SCOPE(_cmd)                                                   \
  nvvk::DebugUtil::ScopedCmdLabel scopedCmdLabel(_cmd, __FUNCTION__)
#define NVVK_DBG_NAME(obj)                                                     \
  nvvk::DebugUtil::getInstance().setObjectName(                                \
      obj, std::string(__FUNCTION__) + ":" #obj "(" +                          \
