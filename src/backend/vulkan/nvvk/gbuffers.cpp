...
{
  assert(m_info.colorFormats.empty() &&
         "Missing deinit()");  // The buffer must be cleared before creating a
                               // new one
...
nvvk::GBuffer::getDescriptorImageInfo(uint32_t i /*= 0*/) const
{
...
VkResult nvvk::GBuffer::initResources(VkCommandBuffer cmd)
{
  ...
  for (uint32_t c = 0; c < numColor; c++)
  {
    ...
    NVVK_FAIL_RETURN(
        m_info.allocator->createImage(m_res.gBufferColor[c], info, viewInfo));
    dutil.setObjectName(m_res.gBufferColor[c].image,
                        "G-Color" + std::to_string(c));
    dutil.setObjectName(m_res.gBufferColor[c].descriptor.imageView,
                        "G-Color" + std::to_string(c));

    // UI Image color view
    viewInfo.image = m_res.gBufferColor[c].image;
    viewInfo.components.a =
        VK_COMPONENT_SWIZZLE_ONE;  // Forcing the VIEW to have a 1 in the alpha
                                   // channel
    NVVK_FAIL_RETURN(
        vkCreateImageView(device, &viewInfo, nullptr, &m_res.uiImageViews[c]));
    dutil.setObjectName(m_res.uiImageViews[c],
                        "UI G-Color" + std::to_string(c));
  ...
  if (m_info.depthFormat != VK_FORMAT_UNDEFINED)
  {
    ...
    NVVK_FAIL_RETURN(m_info.allocator->createImage(m_res.gBufferDepth,
                                                   createInfo, viewInfo));
    ...
    for (uint32_t c = 0; c < numColor; c++)
    {
      ...
      const std::array<VkImageSubresourceRange, 1> range = {
          {{.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT,
            .levelCount = 1,
            .layerCount = 1}}};
      vkCmdClearColorImage(cmd, m_res.gBufferColor[c].image,
                           VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &clear_value,
  ...
  if (m_info.descriptorPool)
  {
    ...
    const VkDescriptorSetLayoutCreateInfo info = {
        ...
        .pBindings = &binding};
    NVVK_FAIL_RETURN(
        vkCreateDescriptorSetLayout(device, &info, nullptr, &m_descLayout));
    ...
    NVVK_FAIL_RETURN(vkAllocateDescriptorSets(device, &allocInfos,
...
[[maybe_unused]] static void usage_GBuffer()
{
  ...
  VkDescriptorPool descriptorPool =
      VK_NULL_HANDLE;  // EX: create a descriptor pool or use the one from the
                       // app (m_app->getTextureDescriptorPool())
  ...
  gbuffer.init(
      {.allocator = &allocator,
       ...
       .depthFormat = VK_FORMAT_D32_SFLOAT,  // EX: use VK_FORMAT_UNDEFINED if
  ...
  // ImGui::Image((ImTextureID)gbuffer.getDescriptorSet(0),
