...
void GraphicsPipelineState::cmdApplyAllStates(VkCommandBuffer cmd) const
{
  ...
  vkCmdSetLineRasterizationModeEXT(
      cmd, rasterizationLineState.lineRasterizationMode);
  if (rasterizationLineState.stippledLineEnable)
  {
    vkCmdSetLineStipple(cmd, rasterizationLineState.lineStippleFactor,
                        rasterizationLineState.lineStipplePattern);
  }

  vkCmdSetRasterizerDiscardEnable(cmd,
  ...
  {
    vkCmdSetDepthBias(cmd, rasterizationState.depthBiasConstantFactor,
  ...
  {
    vkCmdSetDepthBounds(cmd, depthStencilState.minDepthBounds,
  ...
  if (depthStencilState.stencilTestEnable)
  {
    ...
    vkCmdSetStencilOp(
        cmd, VK_STENCIL_FACE_FRONT_BIT, depthStencilState.front.failOp,
        depthStencilState.front.passOp, depthStencilState.front.depthFailOp,
        depthStencilState.front.compareOp);
    vkCmdSetStencilOp(
        cmd, VK_STENCIL_FACE_BACK_BIT, depthStencilState.back.failOp,
        depthStencilState.back.passOp, depthStencilState.back.depthFailOp,
        depthStencilState.back.compareOp);
  }

  vkCmdSetPrimitiveRestartEnable(cmd,
  ...
  vkCmdSetSampleMaskEXT(cmd, multisampleState.rasterizationSamples,
                        &sampleMask);
  ...
  {
    vkCmdSetVertexInputEXT(cmd, static_cast<uint32_t>(vertexBindings.size()),
                           vertexBindings.data(),
                           static_cast<uint32_t>(vertexAttributes.size()),
                           vertexAttributes.data());
  }

  assert(colorWriteMasks.size() == colorBlendEquations.size() &&
         colorWriteMasks.size() == colorBlendEnables.size());

  uint32_t attachmentCount = static_cast<uint32_t>(colorWriteMasks.size());
  if (attachmentCount)
  {
    vkCmdSetColorBlendEquationEXT(cmd, 0, attachmentCount,
                                  colorBlendEquations.data());
    vkCmdSetColorBlendEnableEXT(cmd, 0, attachmentCount,
...
void GraphicsPipelineState::cmdApplyDynamicStates(
    VkCommandBuffer cmd, std::span<const VkDynamicState> dynamicStates) const
{
...
void GraphicsPipelineCreator::addShader(
    VkShaderStageFlagBits stage, const char* pEntryName,
    size_t spirvSizeInBytes, const uint32_t* spirvData,
...
void GraphicsPipelineCreator::addShader(
    ...
    uint32_t subgroupRequiredSize /*= 0*/)
{
  addShader(stage, pEntryName, spirvData.size_bytes(), spirvData.data(),
            pSpecializationInfo, subgroupRequiredSize);
}

VkResult GraphicsPipelineCreator::createGraphicsPipeline(
    VkDevice device, VkPipelineCache cache,
    const GraphicsPipelineState& graphicsState, VkPipeline* pPipeline)
{
  ...
  VkResult result = vkCreateGraphicsPipelines(
      device, cache, 1, &pipelineInfoTemp, nullptr, pPipeline);

  return result;
}

void GraphicsPipelineCreator::buildPipelineCreateInfo(
    VkGraphicsPipelineCreateInfo& createTemp,
    const GraphicsPipelineState& graphicsState)
{
  ...
  assert(graphicsState.vertexInputState.pVertexAttributeDescriptions ==
         nullptr);
  ...
  assert(graphicsState.colorWriteMasks.size() ==
             graphicsState.colorBlendEquations.size() &&
         graphicsState.colorWriteMasks.size() ==
             graphicsState.colorBlendEnables.size());
  ...
  {
    m_renderingState.colorAttachmentCount =
  ...
  m_dynamicState.dynamicStateCount =
      static_cast<uint32_t>(dynamicStateValues.size());
  ...
  m_vertexInputState.pVertexAttributeDescriptions =
      m_staticVertexAttributes.data();
  m_vertexInputDivisorState.pVertexBindingDivisors =
      m_staticVertexBindingDivisors.data();

  m_vertexInputState.vertexBindingDescriptionCount =
      static_cast<uint32_t>(m_staticVertexBindings.size());
  m_vertexInputState.vertexAttributeDescriptionCount =
      static_cast<uint32_t>(m_staticVertexAttributes.size());
  ...
  for (size_t i = 0; i < graphicsState.vertexBindings.size(); i++)
  {
    ...
    m_staticVertexBindings[i].inputRate =
        graphicsState.vertexBindings[i].inputRate;
    ...
    {
      m_staticVertexBindingDivisors[divisorCount].binding =
          graphicsState.vertexBindings[i].binding;
      m_staticVertexBindingDivisors[divisorCount].divisor =
          graphicsState.vertexBindings[i].divisor;
      divisorCount++;
    }
  }
  ...
  {
    m_staticVertexAttributes[i].binding =
        graphicsState.vertexAttributes[i].binding;
    m_staticVertexAttributes[i].format =
        graphicsState.vertexAttributes[i].format;
    m_staticVertexAttributes[i].location =
        graphicsState.vertexAttributes[i].location;
    m_staticVertexAttributes[i].offset =
        graphicsState.vertexAttributes[i].offset;
  }

  m_staticAttachmentState.resize(graphicsState.colorWriteMasks.size());

  m_colorBlendState.attachmentCount =
      static_cast<uint32_t>(m_staticAttachmentState.size());
  ...
  for (size_t i = 0; i < graphicsState.colorWriteMasks.size(); i++)
  {
    ...
    m_staticAttachmentState[i].colorWriteMask =
        graphicsState.colorWriteMasks[i];
    m_staticAttachmentState[i].alphaBlendOp =
        graphicsState.colorBlendEquations[i].alphaBlendOp;
    m_staticAttachmentState[i].colorBlendOp =
        graphicsState.colorBlendEquations[i].colorBlendOp;
    m_staticAttachmentState[i].dstAlphaBlendFactor =
        graphicsState.colorBlendEquations[i].dstAlphaBlendFactor;
    m_staticAttachmentState[i].dstColorBlendFactor =
        graphicsState.colorBlendEquations[i].dstColorBlendFactor;
    m_staticAttachmentState[i].srcAlphaBlendFactor =
        graphicsState.colorBlendEquations[i].srcAlphaBlendFactor;
    m_staticAttachmentState[i].srcColorBlendFactor =
        graphicsState.colorBlendEquations[i].srcColorBlendFactor;
  }

  if (m_shaderStages.size())
  {
    // if we use locally provided shaders, then none must have been provided
    // otherwise
    ...
    for (uint32_t i = 0; i < createTemp.stageCount; i++)
    {
      ...
      {
        m_shaderStageSubgroupSizes[i].pNext =
            const_cast<void*>(m_shaderStages[i].pNext);
...
[[maybe_unused]] static void usage_GraphicsPipeline()
{
  ...
  graphicsState.depthStencilState.depthCompareOp =
  ...
  // example using traditional pipeline
  {
    ...
    // manipulate the public members of the class directly to change the state
    // used for creation
    graphicsPipelineCreator.flags2 =
        VK_PIPELINE_CREATE_2_CAPTURE_STATISTICS_BIT_KHR;

    graphicsPipelineCreator.dynamicStateValues.push_back(
        VK_DYNAMIC_STATE_DEPTH_WRITE_ENABLE);
    graphicsPipelineCreator.dynamicStateValues.push_back(
        VK_DYNAMIC_STATE_DEPTH_COMPARE_OP);

    graphicsPipelineCreator.addShader(VK_SHADER_STAGE_VERTEX_BIT, "main",
                                      vertexCode.size() * sizeof(uint32_t),
                                      vertexCode.data());
    graphicsPipelineCreator.addShader(VK_SHADER_STAGE_FRAGMENT_BIT, "main",
                                      fragmentCode.size() * sizeof(uint32_t),
                                      fragmentCode.data());

    // create the actual pipeline from a combination of state within
    // `graphicsPipelineCreator` and `graphicsState`
    VkPipeline graphicsPipeline;
    VkResult result = graphicsPipelineCreator.createGraphicsPipeline(
        device, nullptr, graphicsState, &graphicsPipeline);

    VkCommandBuffer cmd{};
    VkExtent2D viewportSize{};

    // we recommend (and set defaults) to always use dynamic state for viewport
    // and scissor
  ...
  // example in combination with shader objects
  {
    ...
    // note this is actually the static function as before, but for looks we
    // used the graphicsState
