...
//  vvlInfo.fine_grained_locking = false;  // Customize individual settings as
//  needed vkSetup.instanceCreateInfoExt = vvlInfo.buildPNextChain();
//
//  // Explicit preset selection:
//  nvvk::ValidationSettings vvlInfo{};
//  vvlInfo.setPreset(nvvk::ValidationSettings::LayerPresets::eDebugPrintf);
//  vvlInfo.printf_buffer_size = 4096;   // Customize individual settings as
//  needed vvlInfo.printf_to_stdout = VK_FALSE; // Allow capture
...
struct ValidationSettings
{
  enum class LayerPresets
  {
    ...
    eStandard,         // Good default validation setup that balances validation
                       // coverage and performance
    eReducedOverhead,  // Disables some checks in the interest of better
                       // performance
    eBestPractices,    // Provides warnings on valid API usage that is potential
                       // API misuse
    eSynchronization,  // Identify resource access conflicts due to missing or
  ...
  // VkBool32 debug_disable_spirv_val{VK_FALSE};  // Disable spirv-val (allows
  // normal shader validation to run, but removes just spirv-val for performance
  // reasons)
  ...
  VkBool32 syncval_shader_accesses_heuristic{
      VK_FALSE};  // Shader accesses heuristic
  ...
  VkBool32 gpuav_select_instrumented_shaders{
      VK_FALSE};  // Enable instrumenting shaders selectively
  std::vector<const char*>
      gpuav_shaders_to_instrument{};  // Shader/pipeline name regexes

  // GPU-AV Shader Instrumentation Settings
  VkBool32 gpuav_descriptor_checks{VK_TRUE};  // Descriptors indexing
  VkBool32 gpuav_post_process_descriptor_indexing{
      VK_TRUE};  // Post process descriptor indexing
  VkBool32 gpuav_buffer_address_oob{
      VK_TRUE};  // Out of bounds buffer device addresses
  VkBool32 gpuav_validate_ray_query{VK_TRUE};  // RayQuery SPIR-V instructions
  VkBool32 gpuav_vertex_attribute_fetch_oob{
      VK_TRUE};  // Out of bounds vertex attribute fetching
  ...
  VkBool32 gpuav_indirect_dispatches_buffers{
      VK_TRUE};  // Indirect dispatches parameters
  VkBool32 gpuav_indirect_trace_rays_buffers{
      VK_TRUE};                           // Indirect trace rays parameters
  ...
  // VkBool32 gpuav_debug_disable_all{VK_FALSE};                    // Disable
  // all of GPU-AV VkBool32 gpuav_debug_validate_instrumented_shaders{VK_FALSE};
  // // Validate instrumented shaders VkBool32
  // gpuav_debug_dump_instrumented_shaders{VK_FALSE};      // Dump instrumented
  // shaders uint32_t gpuav_debug_max_instrumentations_count{0};            //
  // Limit how many time a pass can instrument the SPIR-V VkBool32
  // gpuav_debug_print_instrumentation_info{VK_FALSE};     // Print SPIR-V
  // instrumentation info

  // Debug Printf Settings
  VkBool32 printf_only_preset{
      VK_FALSE};  // Debug Printf only preset (a single, quick setting to turn
                  // on only DebugPrintf and turn off everything else)
  ...
  VkBool32 validate_best_practices_arm{
      VK_FALSE};  // ARM-specific best practices
  VkBool32 validate_best_practices_amd{
      VK_FALSE};  // AMD-specific best practices
  VkBool32 validate_best_practices_img{
      VK_FALSE};  // IMG-specific best practices
  VkBool32 validate_best_practices_nvidia{
      VK_FALSE};  // NVIDIA-specific best practices

  // Message and Debug Settings
  std::vector<const char*> debug_action{
      "VK_DBG_LAYER_ACTION_LOG_MSG"};              // Debug Action
  ...
  VkBool32 message_format_display_application_name{
      VK_FALSE};  // Display Application Name
  ...
  void updateSettings()
  {
    ...
    m_layerSettingsCreateInfo.sType =
        VK_STRUCTURE_TYPE_LAYER_SETTINGS_CREATE_INFO_EXT;
    m_layerSettingsCreateInfo.settingCount =
        static_cast<uint32_t>(m_settings.size());
