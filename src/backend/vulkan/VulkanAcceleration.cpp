...
void AccelerationStructures::buildTLAS(const nvsamples::GltfSceneResource& scene)
{
  ...
  for (const shaderio::GltfInstance& instance : scene.instances)
  {
    ...
    ray_inst.instanceShaderBindingTableRecordOffset = 0;  // Same hit group for all objects
...
AccelerationStructures::primitiveToGeometry(const shaderio::GltfMesh& mesh)
{
  ...
  VkAccelerationStructureGeometryTrianglesDataKHR triangles{
      ...
      .vertexData = {.deviceAddress = VkDeviceAddress(mesh.gltfBuffer) + triMesh.positions.offset},
