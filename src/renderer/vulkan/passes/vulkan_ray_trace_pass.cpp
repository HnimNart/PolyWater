#include "vulkan_ray_trace_pass.hpp"
...
/**********************************************************/
void VulkanRayTracePass::deinit()
/**********************************************************/
{
...
/**********************************************************/
void VulkanRayTracePass::setup(PassBuilder& builder)
/**********************************************************/
{
...
/**********************************************************/
void VulkanRayTracePass::createPipeline()
/**********************************************************/
{
...
/**********************************************************/
void VulkanRayTracePass::createRayTracingPipeline()
/**********************************************************/
{
...
/**********************************************************/
void VulkanRayTracePass::createDescriptorLayout()
/**********************************************************/
{
  SCOPED_TIMER_FUNC();
...
/**********************************************************/
void VulkanRayTracePass::createPipelineSBT()
/**********************************************************/
{
  SCOPED_TIMER_FUNC();
  ...
  m_shaderCode.push_back(
      VulkanSlangCompiler::instance().compile(raygen.filename, raygen.spirv));
  stages.push_back(
  ...
  m_shaderCode.push_back(
      VulkanSlangCompiler::instance().compile(miss.filename, miss.spirv));
  stages.push_back(
      {.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO,
       .pNext = &m_shaderCode.back(),
  ...
  m_shaderCode.push_back(
      VulkanSlangCompiler::instance().compile(missShadow.filename, missShadow.spirv));
  ...
  for (auto& [type, entry] : m_shaderManager->getRegistry())
  {
    ...
    m_shaderCode.push_back(VulkanSlangCompiler::instance().compile(entry.filename));
...
/**********************************************************/
void VulkanRayTracePass::createShaderBindingTable(
...
/**********************************************************/
void VulkanRayTracePass::execute(const IRenderContext& ctx)
/**********************************************************/
{
