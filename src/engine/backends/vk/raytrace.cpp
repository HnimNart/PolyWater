...
void Raytracer::setup(const VkDevice& device, const VkPhysicalDevice& physicalDevice,
                      nvvk::ResourceAllocator* allocator, uint32_t queueFamily)
{
  m_device = device;
...
void Raytracer::createTopLevelAS(std::vector<ObjInstance>& instances, ImplInst& implicitObj)
{
  ...
  for (uint32_t i = 0; i < nbObj; i++)
  {
    ...
    rayInst.transform =
        nvvk::toTransformMatrixKHR(instances[i].transform);  // Position of the instance
    rayInst.instanceCustomIndex = instances[i].objIndex;     // gl_InstanceCustomIndexEXT
    rayInst.accelerationStructureReference =
        m_rtBuilder.getBlasDeviceAddress(instances[i].objIndex);
  ...
  if (!implicitObj.objImpl.empty())
  {
    VkAccelerationStructureInstanceKHR rayInst{};
    rayInst.transform =
        nvvk::toTransformMatrixKHR(implicitObj.transform);  // Position of the instance
    rayInst.instanceCustomIndex = instances[nbObj].objIndex;
    rayInst.accelerationStructureReference =
        m_rtBuilder.getBlasDeviceAddress(static_cast<uint32_t>(implicitObj.blasId));
    rayInst.flags = VK_GEOMETRY_INSTANCE_TRIANGLE_FACING_CULL_DISABLE_BIT_KHR;
    rayInst.mask = 0xFF;  // Only be hit if rayMask & instance.mask != 0
    rayInst.instanceShaderBindingTableRecordOffset =
        1;  // We will use the same hit group for all objects (the second one)
    tlas.emplace_back(rayInst);
  }
...
void Raytracer::createRtDescriptorSet(const VkImageView& outputImage)
{
  ...
  writes.emplace_back(
      m_rtDescSetLayoutBind.makeWrite(m_rtDescSet, RtxBindings::eTlas, &descASInfo));
  writes.emplace_back(
      m_rtDescSetLayoutBind.makeWrite(m_rtDescSet, RtxBindings::eOutImage, &imageInfo));
...
void Raytracer::createRtPipeline(VkDescriptorSetLayout& sceneDescLayout)
{
  ...
  stages[eMiss] = stage;
  // The second miss shader is invoked when a shadow ray misses the geometry. It simply indicates
  // that no occlusion has been found
  stage.module = nvvk::createShaderModule(
  ...
  // hit points of the camera rays, hence a recursion level of 2. This number should be kept as
  // low as possible for performance reasons. Even recursive ray tracing should be flattened into
  // a loop in the ray generation to avoid deep recursion.
...
//
void Raytracer::raytrace(const VkCommandBuffer& cmdBuf, const glm::vec4& clearColor,
                         VkDescriptorSet& sceneDescSet, VkExtent2D& size,
