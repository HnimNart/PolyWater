...
void Offscreen::setup(const VkDevice& device, const VkPhysicalDevice& physicalDevice,
...
void Offscreen::createFramebuffer(const VkExtent2D& size)
{
  ...
  // Creating the color image
  {
    auto colorCreateInfo =
        nvvk::makeImage2DCreateInfo(size, m_colorFormat,
  ...
  {
    auto depthCreateInfo = nvvk::makeImage2DCreateInfo(size, m_depthFormat,
  ...
  // Setting the image layout for both color and depth
  {
    ...
    nvvk::cmdBarrierImageLayout(cmdBuf, m_colorTexture.image, VK_IMAGE_LAYOUT_UNDEFINED,
  ...
  std::vector<VkImageView> attachments = {m_colorTexture.descriptor.imageView,
...
{
  m_dsetLayoutBinding.addBinding(0, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1,
                                 VK_SHADER_STAGE_FRAGMENT_BIT);
...
void Offscreen::draw(VkCommandBuffer cmdBuf, const VkExtent2D& size)
{
  ...
  vkCmdPushConstants(cmdBuf, m_pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(float),
                     &aspectRatio);
  vkCmdBindPipeline(cmdBuf, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipeline);
  vkCmdBindDescriptorSets(cmdBuf, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipelineLayout, 0, 1, &m_dset,
                          0, nullptr);
