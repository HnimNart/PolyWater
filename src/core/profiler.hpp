...
//   Any frame section must be triggered within a `frameBegin`/`frameEnd`
//   pairing.
...
public:
  ...
  struct GpuTimeProvider
  {
    ...
    // Utility functions that help converting the FrameSectionID or
    // AsyncSectionID to a linear array index. Useful for GPU timer classes to
    // manage resources as big arrays, or pools of arrays.
  ...
  // GPU timer implementations may want to use this function to reset the cpu
  // time to exclude internal setup overhead
  ...
  // ```
  // // by default accumulates all "blubb" timers in the loop on the same
  // nesting level for (..) {
  ...
  // GPU profilers may want to reset the cpu time to exclude internal setup
  // overhead
  ...
  // early heavier frames after configuration changes) implicitly resets are
