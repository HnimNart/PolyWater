...
static void addTriangle(PrimitiveMesh &mesh, uint32_t a, uint32_t b,
                        uint32_t c) {
...
static void generateFacetedNormals(PrimitiveMesh &mesh) {
  ...
  for (int i = 0; i < num_indices; i++) {
    ...
    glm::vec3 n = glm::normalize(glm::cross(glm::normalize(v1.pos - v0.pos),
                                            glm::normalize(v2.pos - v0.pos)));
...
// Generates a flat plane mesh with the specified number of steps, width, and
// depth. The plane is essentially a grid with the specified number of
...
PrimitiveMesh createPlane(int steps, float width, float depth) {
  ...
  for (int sz = 0; sz <= steps; sz++) {
    for (int sx = 0; sx <= steps; sx++) {
      ...
      v.pos = glm::vec3(-0.5F + (static_cast<float>(sx) * increment), 0.0F,
                        -0.5F + (static_cast<float>(sz) * increment));
      v.pos *= glm::vec3(width, 1.0F, depth);
      v.nrm = glm::vec3(0.0F, 1.0F, 0.0F);
      v.tex =
          glm::vec2(static_cast<float>(sx) / static_cast<float>(steps),
...
PrimitiveMesh createCube(float width /*= 1*/, float height /*= 1*/,
  ...
  std::vector<glm::vec3> pnt = {{-s.x, -s.y, -s.z}, {-s.x, -s.y, s.z},
                                {-s.x, s.y, -s.z},  {-s.x, s.y, s.z},
                                {s.x, -s.y, -s.z},  {s.x, -s.y, s.z},
                                {s.x, s.y, -s.z},   {s.x, s.y, s.z}};
  std::vector<glm::vec3> nrm = {{-1.0F, 0.0F, 0.0F}, {0.0F, 0.0F, 1.0F},
...
// Generates a UV-sphere mesh with the specified radius, number of sectors
...
// Generates a sphere mesh with the specified radius and subdivisions (level of
// detail). It uses the icosahedron subdivision technique to iteratively refine
// the mesh by subdividing triangles into smaller triangles to approximate a
// more spherical shape. It calculates vertex positions, normals, and texture
...
PrimitiveMesh createSphereMesh(float radius, int subdivisions) {
  ...
  std::vector<glm::vec3> vertices = {{-1, t, 0},  {1, t, 0},   {-1, -t, 0},
                                     {1, -t, 0},  {0, -1, t},  {0, 1, t},
                                     {0, -1, -t}, {0, 1, -t},  {t, 0, -1},
                                     {t, 0, 1},   {-t, 0, -1}, {-t, 0, 1}};

  // Function to calculate the midpoint between two vertices
  auto midpoint = [](const glm::vec3 &v1, const glm::vec3 &v2) {
    return (v1 + v2) * 0.5f;
  };

  auto texCoord = [](const glm::vec3 &v1) {
    return glm::vec2{0.5f + std::atan2(v1.z, v1.x) / (2 * M_PI),
                     0.5f - std::asin(v1.y) / M_PI};
  };
  ...
  for (int i = 0; i < subdivisions; ++i) {
    ...
    for (const auto &tri : triangles) {
      ...
      glm::vec3 mid1 = midpoint(primitiveVertices[tri.indices[0]].pos,
                                primitiveVertices[tri.indices[1]].pos);
      glm::vec3 mid2 = midpoint(primitiveVertices[tri.indices[1]].pos,
                                primitiveVertices[tri.indices[2]].pos);
      glm::vec3 mid3 = midpoint(primitiveVertices[tri.indices[2]].pos,
                                primitiveVertices[tri.indices[0]].pos);
      ...
      primitiveVertices.push_back(
          {mid1Normalized * radius, mid1Normalized, mid1Uv});
      primitiveVertices.push_back(
          {mid2Normalized * radius, mid2Normalized, mid2Uv});
      primitiveVertices.push_back(
          {mid3Normalized * radius, mid3Normalized, mid3Uv});
...
// majorRadius: This represents the distance from the center of the torus to the
// center of the tube (the larger circle's radius). minorRadius: This represents
// the radius of the tube (the smaller circle's radius). majorSegments: The
...
PrimitiveMesh createTorusMesh(float majorRadius, float minorRadius,
  ...
  for (int i = 0; i <= majorSegments; ++i) {
    ...
    glm::vec3 center = {majorRadius * std::cos(angle1), 0.0f,
                        majorRadius * std::sin(angle1)};

    for (int j = 0; j <= minorSegments; ++j) {
      float angle2 = j * minorStep;
      glm::vec3 position = {
          center.x + minorRadius * std::cos(angle2) * std::cos(angle1),
          minorRadius * std::sin(angle2),
          center.z + minorRadius * std::cos(angle2) * std::sin(angle1)};

      glm::vec3 normal = {std::cos(angle2) * std::cos(angle1), std::sin(angle2),
                          std::cos(angle2) * std::sin(angle1)};

      glm::vec2 texCoord = {static_cast<float>(i) / majorSegments,
...
PrimitiveMesh mergeNodes(const std::vector<Node> &nodes,
  ...
  for (const auto &n : nodes) {
    ...
    const PrimitiveMesh &mesh = meshes[n.mesh];
...
// Takes a 3D mesh as input and modifies its vertices by adding random
...
// Takes a 3D mesh as input and returns a new mesh with duplicate vertices
// removed. This function iterates through each triangle in the original
// PrimitiveMesh, compares its vertices, and creates a new set of unique
...
PrimitiveMesh removeDuplicateVertices(const PrimitiveMesh &mesh,
  ...
  auto hash = [&](const PrimitiveVertex &v) {
    if (testNormal) {
      if (testUv)
        return hashVal(v.pos.x, v.pos.y, v.pos.z, v.nrm.x, v.nrm.y, v.nrm.z,
                       v.tex.x, v.tex.y);
      else
        return hashVal(v.pos.x, v.pos.y, v.pos.z, v.nrm.x, v.nrm.y, v.nrm.z);
    } else if (testUv)
      return hashVal(v.pos.x, v.pos.y, v.pos.z, v.tex.x, v.tex.y);
    return hashVal(v.pos.x, v.pos.y, v.pos.z);
  };
  auto equal = [&](const PrimitiveVertex &l, const PrimitiveVertex &r) {
    return (l.pos == r.pos) && (testNormal ? l.nrm == r.nrm : true) &&
           (testUv ? l.tex == r.tex : true);
  };
  std::unordered_map<PrimitiveVertex, uint32_t, decltype(hash), decltype(equal)>
      vertexIndexMap(0, hash, equal);
