...
Bbox::Bbox(const std::vector<glm::vec3>& corners)
{
  for (auto& c : corners)
  {
    insert(c);
...
void Bbox::insert(const glm::vec3& v)
{
...
void Bbox::insert(const Bbox& b)
{
  insert(b.m_min);
  insert(b.m_max);
}

uint32_t Bbox::rank() const
{
  uint32_t result{0};
...
Bbox Bbox::transform(glm::mat4 mat)
{
