                                                ...
                                                bool full = false) const
{
  ...
  for (size_t i = 0; i < timerInfos.size(); i++)
  {
    ...
    const char* apiName =
        !timerApiNames[i].empty() ? timerApiNames[i].c_str() : "N/A";
    const char* timerName =
        !timerNames[i].empty() ? timerNames[i].c_str() : "N/A";
    ...
    {
      stats += fmt::format(
          "Timeline \"{}\"; level {}; Timer \"{}\"; GPU; avg {}; min {}; max "
          "{}; last {}; CPU; avg {}; min {}; max {}; last {}; samples {};\n",
          name, int32_t(info.async ? -1 : info.level), timerNames[i],
          (uint32_t) (info.gpu.average), (uint32_t) (info.gpu.absMinValue),
          (uint32_t) (info.gpu.absMaxValue), (uint32_t) (info.gpu.last),
          (uint32_t) (info.cpu.average), (uint32_t) (info.cpu.absMinValue),
          (uint32_t) (info.cpu.absMaxValue), (uint32_t) (info.cpu.last),
          info.numAveraged);
    }
    else
    {
      stats += fmt::format("{:12}; {:3};{}{:16}{}; GPU; avg {:6}; CPU; avg "
                           "{:6}; microseconds;\n",
                           name, int32_t(info.async ? -1 : info.level),
                           indentSpaces, timerNames[i], indentSpacesOp,
                           (uint32_t) (info.gpu.average),
...
void ProfilerTimeline::frameEnd()
{
  ...
  if (m_frame.sectionsCount &&
      m_frame.sectionsCount != m_frame.sectionsCountLast)
  {
  ...
  if ((m_frame.count - m_frame.countLastReset) > m_info.frameDelay)
  {
    ...
    for (uint32_t i = 0; i < m_frame.sectionsCount; i++)
    {
      ...
      bool available =
          !section.gpuTimeProvider || section.gpuTimeProvider->frameFunction(
                                          sec, section.gpuTimes[queryFrame]);
...
void ProfilerTimeline::frameInternalSnapshot()
{
  ...
  for (uint32_t i = 0; i < m_frame.sectionsCountLast; i++)
  {
    ...
    if (frameGetTimerInfo(i, info))
    {
      ...
      m_frameSnapshot.timerApiNames.push_back(
          section.gpuTimeProvider ? section.gpuTimeProvider->apiName : "");
    }
  }
}
...
ProfilerTimeline::FrameSectionID ProfilerTimeline::frameGetSectionID()
{
  ...
  {
    grow(m_frame.sections, m_frame.sections.size() * 2,
         m_frame.averagingCountLast);
  }
                                    ...
                                    GpuTimeProvider* gpuTimeProvider)
{
  ...
  if (section.name != name || section.gpuTimeProvider != gpuTimeProvider ||
      section.level != level)
  {
    section.name = name;
...
bool ProfilerTimeline::asyncGetTimerInfo(uint32_t i, TimerInfo& info) const
{
  ...
  double gpuTime = 0;
  bool available = (!section.gpuTimeProvider ||
                    section.gpuTimeProvider->asyncFunction(sectionID, gpuTime));
...
void ProfilerTimeline::getAsyncSnapshot(Snapshot& snapShot) const
{
  ...
  for (uint32_t i = 0; i < m_async.sectionsCount; i++)
  {
    ...
    if (asyncGetTimerInfo(i, timerInfo))
    {
      ...
      snapShot.timerApiNames.push_back(
          section.gpuTimeProvider ? section.gpuTimeProvider->apiName : "");
    }
  }
...
[[maybe_unused]] static void usage_Profiler()
{
  ...
  // Note that querying is always done in a non-blocking way if GPU timers are
  // involved. Therefore the results might be from past frames, or incomplete if
  // they haven't finished yet.

  // Async event timers are queried at snapshot time, whilst frame event timers
  // are queried at end of frame.
