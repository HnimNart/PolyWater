...
public:
  ...
  explicit ProgressBar(const std::string& name, int width = 50) : m_name(name), m_width(width) {}

  void update(uint32_t current, uint32_t total)
  {
    float progress = 0.0f;
    if (total > 0)
      progress = std::clamp(static_cast<float>(current) / static_cast<float>(total), 0.0f, 1.0f);
    ...
    std::printf("] %3d%% (%d/%d) [%s < %s]   ", static_cast<int>(progress * 100.0f), current, total,
