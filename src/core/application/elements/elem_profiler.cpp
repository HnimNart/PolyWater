...
core::ElementProfiler::ElementProfiler(nvutils::ProfilerManager* profiler, std::shared_ptr<ViewSettings> defaultViewSettings)
    : m_profiler(profiler)
{
  m_views.push_back({.state = defaultViewSettings ? defaultViewSettings : std::make_shared<core::ElementProfiler::ViewSettings>()});
};
...
uint32_t core::ElementProfiler::addEntries(const nvutils::ProfilerTimeline::Snapshot& snapshot,
...
void core::ElementProfiler::displayTableNode(const EntryNode& node, bool detailed, uint32_t defaultOpenLevels, uint32_t depth)
{
...
void core::ElementProfiler::renderPieChartNode(const EntryNode& node, int level, int numLevels, double plotRadius, double angle0, double totalTime)
{
  // Gather data
