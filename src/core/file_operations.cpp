...
std::filesystem::path core2::getExecutablePath() {
  ...
  const ssize_t count =
      readlink("/proc/self/exe", buffer, sizeof(buffer) / sizeof(buffer[0]));
...
std::string core2::utf8FromPath(const std::filesystem::path &path) noexcept {
  try {
    ...
    // WideCharToMultiByte because
    // - <codecvt> is deprecated and scheduled to be removed from C++ (see
    ...
    std::ignore =
        WideCharToMultiByte(CP_UTF8, 0, utf16Str, utf16CharactersI,
...
std::filesystem::path core2::pathFromUtf8(const char *utf8) noexcept {
  ...
  try {
    ...
    static_assert(
        std::is_same_v<std::filesystem::path::string_type, std::wstring>);
    std::wstring result(utf16Characters, wchar_t(0));
    std::ignore =
        MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, utf8, utf8BytesI,
...
bool core2::extensionMatches(const std::filesystem::path &path,
  ...
  // path). We can bring this down to 1 on Windows and 0 on Linux with a custom
  // implementation.
