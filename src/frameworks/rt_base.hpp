...
#include "slang.h"
#include "src/gltf_utils.hpp"  // GLTF utilities for loading and importing GLTF models
#include "src/path_utils.hpp"  // Path utilities for handling resources file paths
#include "src/utils.hpp"       // Common utilities for the sample application
...
public:
  ...
  void onAttach(nvapp::Application* app) override
  {
    ...
    createGraphicsDescriptorSetLayout();  // Create the descriptor set layout for the graphics
                                          // pipeline
  ...
  bool renderUI()
  {
    ...
    {
      ImGui::Image(ImTextureID(m_gBuffers.getDescriptorSet(eImgTonemapped)),
    ...
    if (ImGui::Begin("Settings"))
    {
      ...
      if (ImGui::CollapsingHeader("Lighting"))
      {
        ...
        else
        {
          ...
          {
            modified |= PE::DragFloat3(
                "Light Position", glm::value_ptr(sceneInfo.punctualLights[0].position), 1.0, -20.0f,
          ...
          if (sceneInfo.punctualLights[0].type == shaderio::GltfLightType::eDirectional ||
              sceneInfo.punctualLights[0].type == shaderio::GltfLightType::eSpot)
          {
            modified |= PE::SliderFloat3(
                "Light Direction", glm::value_ptr(sceneInfo.punctualLights[0].direction), -1.0f,
                1.0f, "%.2f", ImGuiSliderFlags_None, "Direction of the light");
          }

          modified |= PE::SliderFloat("Light Intensity", &sceneInfo.punctualLights[0].intensity,
                                      0.0f, 1000.0f, "%.2f", ImGuiSliderFlags_Logarithmic,
                                      "Intensity of the light");
          modified |=
              PE::ColorEdit3("Light Color", glm::value_ptr(sceneInfo.punctualLights[0].color),
                             ImGuiColorEditFlags_NoInputs, "Color of the light");
          modified |= PE::Combo("Light Type", (int*) &sceneInfo.punctualLights[0].type,
          ...
          {
            modified |=
                PE::SliderAngle("Cone Angle", &sceneInfo.punctualLights[0].coneAngle, 0.f, 90.f,
                                "%.2f", ImGuiSliderFlags_None, "Cone angle of the spot light");
          }
          PE::end();
        }
      }
      ...
      ImGui::Separator();
      PE::begin();
      modified |= PE::SliderFloat2(
          "Metallic/Roughness Override", glm::value_ptr(m_metallicRoughnessOverride), -0.01f, 1.0f,
  ...
  void createGraphicsDescriptorSetLayout()
  {
    ...
    m_descPack.init(m_bindings, m_app->getDevice(), 1,
  ...
  virtual void createGraphicsPipelineLayout()
  {
    ...
    NVVK_CHECK(vkCreatePipelineLayout(m_app->getDevice(), &pipelineLayoutInfo, nullptr,
  ...
  void updateSceneBuffer(VkCommandBuffer cmd)
  {
    ...
    m_sceneResource.sceneInfo.viewProjMatrix =
        projMatrix * viewMatrix;  // Combine the view and projection matrices
    m_sceneResource.sceneInfo.projInvMatrix =
    ...
    m_sceneResource.sceneInfo.instances =
        (shaderio::GltfInstance*)
            m_sceneResource.bInstances.address;  // Get the address of the instance buffer
    m_sceneResource.sceneInfo.meshes =
        (shaderio::GltfMesh*)
            m_sceneResource.bMeshes.address;  // Get the address of the mesh buffer
    m_sceneResource.sceneInfo.materials =
        (shaderio::GltfMetallicRoughness*)
            m_sceneResource.bMaterials.address;  // Get the address of the material buffer
    ...
    vkCmdUpdateBuffer(cmd, m_sceneResource.bSceneInfo.buffer, 0, sizeof(shaderio::GltfSceneInfo),
  ...
  //
  virtual nvvk::AccelerationStructureGeometryInfo
  primitiveToGeometry(const shaderio::GltfMesh& gltfMesh)
  {
  ...
  virtual void createRaytraceDescriptorLayout()
  {
    ...
    m_rtDescPack.init(m_rtBindings, m_app->getDevice(), 0,
                      VK_DESCRIPTOR_SET_LAYOUT_CREATE_PUSH_DESCRIPTOR_BIT_KHR);
  }

  // Create Ray Trace Pipeline
  VkRayTracingPipelineCreateInfoKHR createRayTracingPipelineCreateInfo(
      std::span<const VkPipelineShaderStageCreateInfo> stages,
      std::span<const VkRayTracingShaderGroupCreateInfoKHR> shaderGroups, uint32_t depth = 2)
  {
    ...
    vkCreatePipelineLayout(m_app->getDevice(), &pipelineLayoutCreateInfo, nullptr,
                           &m_rtPipelineLayout);
    ...
    VkRayTracingPipelineCreateInfoKHR rtPipelineInfo{
        ...
        .maxPipelineRayRecursionDepth =
            std::max(2U, m_rtProperties.maxRayRecursionDepth),  // Ray depth
  ...
  virtual void raytraceScene(VkCommandBuffer cmd)
  {
    ...
    m_pushValues.sceneInfoAddress =
        (shaderio::GltfSceneInfo*) m_sceneResource.bSceneInfo
            .address;  // Pass the address of the scene information buffer to the shader
    m_pushValues.metallicRoughnessOverride =
  ...
  virtual void rasterScene(VkCommandBuffer cmd)
  {
    ...
    if (m_sceneResource.sceneInfo.useSky)
    {
      ...
      m_skySimple.runCompute(cmd, m_app->getViewportSize(), viewMatrix, projMatrix,
                             m_sceneResource.sceneInfo.skySimpleParam,
                             m_gBuffers.getDescriptorImageInfo(eImgRendered));
    }
    ...
    colorAttachment.loadOp =
        m_sceneResource.sceneInfo.useSky ? VK_ATTACHMENT_LOAD_OP_LOAD : VK_ATTACHMENT_LOAD_OP_CLEAR;
    ...
    if (m_vertexShader && m_fragmentShader)
    {
      m_dynamicPipeline.cmdBindShaders(cmd,
                                       {.vertex = m_vertexShader, .fragment = m_fragmentShader});
    }
    ...
    for (size_t i = 0; i < m_sceneResource.instances.size(); i++)
    {
      ...
      pushValues.normalMatrix =
          glm::transpose(glm::inverse(glm::mat3(m_sceneResource.instances[i].transform)));
...
protected:
  ...
  nvvk::StagingUploader m_stagingUploader{};  // Utility to upload data to the GPU, used for staging
                                              // buffers and images
  nvvk::SamplerPool
      m_samplerPool{};         // Texture sampler pool, used to acquire texture samplers for images
  ...
  nvvk::DescriptorBindings m_bindings;  // The descriptor bindings used to create the descriptor set
                                        // layout and descriptor sets
  ...
  glm::vec2 m_metallicRoughnessOverride{
      -0.01f, -0.01f};  // Override values for metallic and roughness, used in the UI to control the
                        // material properties
