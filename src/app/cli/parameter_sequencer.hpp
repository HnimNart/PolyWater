...
// and the profiler uses a window of `profilerAverageCount` many frames for
...
public:
  ...
  public:
    ...
    // these are registered and added to the `parameterParser` and
  ...
  // alive). The sequence profiling results are queried from the provided
  // `profilerManager` (must be kept alive). It is possible to run without a
  // provided profilerManager and then simply execute the script.
  //
  ...
  // the sequence script. When a previous sequence completed it will query the
  // profiler for a string of the statistics in `full` detail mode and the log
  // them via `LogLevel::eSTATS
  //
  // Returns `true` if the sequences were completed and no more frames are
  // required.
  bool prepareFrame();
