...
// The ParameterRegistry serves as a central place to register tweakable
// parameters within an application. This allows to reduce describing such
// parameters multiple times redundantly, for example for command line arguments
// or GUI. A Parameter contains a pointer to the destination variable that must
// be valid while the Parameter is used.
//
// All Parameters must be constructed through the registry and their lifetime is
// linked to the lifetime of the registry. They cannot be destroyed
// individually.
//
...
public:
  ...
  const Parameter<bool> *add(const ParameterBase::Info &info,
                             bool *destination);

  // if this parameter is parsed, it will always force setting the provided
  // `triggerValue`
  const Parameter<bool> *add(const ParameterBase::Info &info, bool *destination,
                             bool triggerValue);

  const Parameter<std::string> *add(const ParameterBase::Info &info,
                                    std::string *destination);
  const Parameter<std::filesystem::path> *
  add(const ParameterBase::Info &info, std::filesystem::path *destination);

  // special filename that can be triggered without parameter name based on
  // argument extension alone extensions must be provided in lowercase
  const Parameter<std::filesystem::path> *
  add(const ParameterBase::Info &info,
      const std::vector<std::string> &extensions,
      std::filesystem::path *destination);

  // if `extensions` are provided then parser can trigger this based on string
  // suffix alone, rather than requiring the parameter name in that case
  // argCount must be 1 and extensions must be provided in lowercase
  const ParameterBase *
  addCustom(const ParameterBase::Info &info, uint32_t argCount,
            ParameterBase::CallbackCustom custom,
            const std::vector<std::string> &extensions = {});

  const Parameter<float> *
  add(const ParameterBase::Info &info, float *destination,
  ...
  const ParameterBase *addArray(const ParameterBase::Info &info,
                                uint32_t arrayLength, float *destination,
                                const float *minValues = nullptr,
                                const float *maxValues = nullptr);

  const Parameter<int32_t> *
  add(const ParameterBase::Info &info, int32_t *destination,
  ...
  add(const ParameterBase::Info &info, int16_t *destination,
  ...
  add(const ParameterBase::Info &info, int8_t *destination,
  ...
  const Parameter<uint32_t> *
  add(const ParameterBase::Info &info, uint32_t *destination,
      const uint32_t minValue = std::numeric_limits<uint32_t>::min(),
      const uint32_t maxValue = std::numeric_limits<uint32_t>::max());

  const Parameter<uint16_t> *
  add(const ParameterBase::Info &info, uint16_t *destination,
  ...
  add(const ParameterBase::Info &info, uint8_t *destination,
  ...
  // convenience wrapper to automatically infer the vector dimensions from a glm
  // vector type
  ...
  addVector(const ParameterBase::Info &info, GLMVEC *destination,
            ...
            GLMVEC maxValue = GLMVEC(
                std::numeric_limits<typename GLMVEC::value_type>::max())) {
    static_assert(sizeof(GLMVEC) <= sizeof(ParameterBase::MinMaxData));
    return static_cast<const Parameter<GLMVEC> *>(addArray(
        info, GLMVEC::length(), (typename GLMVEC::value_type *)destination,
  ...
  const Parameter<GLMMAT> *addMatrix(const ParameterBase::Info &info,
    ...
    return static_cast<const Parameter<GLMMAT> *>(
        addArray(info, GLMMAT::length() * (GLMMAT::col_type::length()),
                 (typename GLMMAT::value_type *)destination, nullptr, nullptr));
  }

protected:
  ParameterBase *addNewBase(const ParameterBase::Info &info,
                            ParameterBase::Type type, uint32_t argCount,
                            void *destination);

  // Contains the pointers to all allocated parameters.
  // We allocate parameters individually so their pointers can be used more
  // persistently.
