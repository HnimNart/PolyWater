...
ParameterParser::ParameterParser(
  ...
  add(m_builtinRegistry.addCustom(
      {
          .name = "configfile",
          .help = "(string) - Parses provided config file. Relative filenames "
                  "are allowed.",
          .shortName = "cf",
      },
  ...
  add(m_builtinRegistry.addCustom(
      {
          .name = "help",
          .help = "() - Prints all known parameter options registered to the "
                  "parser.",
          .shortName = "h",
      },
      0, fnHelp));
...
void ParameterParser::printHelp() const {
  ...
  for (const ParameterBase *parameter : m_parsedParameters) {
    ...
    // Print the help for each argument. This is a bit more involved since we do
