...
const Parameter<int16_t> *ParameterRegistry::add(
    const ParameterBase::Info &info, int16_t *destination,
...
const Parameter<int32_t> *ParameterRegistry::add(
    const ParameterBase::Info &info, int32_t *destination,
...
const Parameter<uint8_t> *ParameterRegistry::add(
    const ParameterBase::Info &info, uint8_t *destination,
...
ParameterRegistry::addArray(const ParameterBase::Info &info,
  ...
  ParameterBase *parameter =
      addNewBase(info, ParameterBase::Type::FLOAT32, arrayLength, destination);
  for (uint32_t i = 0; i < arrayLength; i++) {
    parameter->minMaxValues[0].f32[i] =
        minValues ? minValues[i] : std::numeric_limits<float>::min();
    parameter->minMaxValues[1].f32[i] =
        maxValues ? maxValues[i] : std::numeric_limits<float>::min();
  }
...
ParameterRegistry::addArray(const ParameterBase::Info &info,
  ...
  ParameterBase *parameter =
      addNewBase(info, ParameterBase::Type::INT32, arrayLength, destination);
  for (uint32_t i = 0; i < arrayLength; i++) {
    parameter->minMaxValues[0].i32[i] =
        minValues ? minValues[i] : std::numeric_limits<int32_t>::min();
    parameter->minMaxValues[1].i32[i] =
        maxValues ? maxValues[i] : std::numeric_limits<int32_t>::max();
  }
...
ParameterRegistry::addArray(const ParameterBase::Info &info,
  ...
  ParameterBase *parameter =
      addNewBase(info, ParameterBase::Type::UINT32, arrayLength, destination);
  for (uint32_t i = 0; i < arrayLength; i++) {
    parameter->minMaxValues[0].u32[i] =
        minValues ? minValues[i] : std::numeric_limits<uint32_t>::min();
    parameter->minMaxValues[1].u32[i] =
        maxValues ? maxValues[i] : std::numeric_limits<uint32_t>::max();
  }
...
ParameterRegistry::addCustom(const ParameterBase::Info &info, uint32_t argCount,
  ...
  ParameterBase *parameter =
      addNewBase(info, ParameterBase::Type::CUSTOM, argCount, nullptr);
