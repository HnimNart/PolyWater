...
void ElementGpuMonitor::imguiGraphLines(uint32_t gpuIndex)
{
  ...
  std::string lineString =
      fmt::format("Load: {}%", deviceUtilization.gpuUtilization.get()[offset]);
  float memUsage = static_cast<float>(deviceMemory.memoryUsed.get()[offset]) /
                   deviceMemory.memoryTotal.get() * 100.f;
  std::string memString =
      fmt::format("Memory: {}%", static_cast<int>(memUsage));

  static ImPlotFlags s_plotFlags = ImPlotFlags_NoBoxSelect |
                                   ImPlotFlags_NoMouseText |
                                   ImPlotFlags_Crosshairs;
  static ImPlotAxisFlags s_axesFlags =
                        ...
                        s_plotFlags))
  {
    ...
    ImPlot::SetupAxis(ImAxis_Y2, "Mem",
    ...
    if (ImPlot::IsPlotHovered())
    {
      ...
      ImGui::BeginTooltip();
      ImGui::Text("Load: %d%%",
                  deviceUtilization.gpuUtilization.get()[gpuOffset]);
...
void ElementGpuMonitor::imguiProgressBars()
{
       ...
       gpuIndex++)  // Number of gpu
  {
    ...
    std::string progtext = fmt::format(
        "{:.2f}/{:.2f} GiB",
        static_cast<double>(deviceMemoryInfo.memoryUsed.get()[offset]) /
            GiBvalue,
...
void ElementGpuMonitor::imguiDeviceInfo(uint32_t deviceIndex)
{
              ...
              ImGuiTableFlags_RowBg))
  {
    ...
    imguiNvmlField(deviceInfo.computeCapabilityMinor,
    ...
    imguiNvmlField(deviceInfo.clockDefaultGraphics, "Default clock graphics",
                   "MHz");
    imguiNvmlField(deviceInfo.clockMaxGraphics, "Max clock graphics", "MHz");
    imguiNvmlField(deviceInfo.clockBoostGraphics, "Boost clock graphics",
                   "MHz");
    ...
    imguiNvmlField(deviceInfo.supportedClocksThrottleReasons,
                   "Supported clock throttle reasons");
    ...
    imguiNvmlField(deviceInfo.tempThresholdShutdown,
                   "Temperature threshold HW Shutdown", "C");
    imguiNvmlField(deviceInfo.tempThresholdHWSlowdown,
                   "Temperature threshold HW Slowdown", "C");
    imguiNvmlField(deviceInfo.tempThresholdSWSlowdown,
                   "Temperature threshold SW Slowdown", "C");
    imguiNvmlField(deviceInfo.tempThresholdDropBelowBaseClock,
                   "Temperature threshold before dropping below base clocks",
                   "C");
...
void ElementGpuMonitor::imguiDeviceMemory(uint32_t deviceIndex)
{
  ...
  std::string bar1Line = fmt::format(
      "BAR1: {}MiB ({}%)", memory.bar1Used.get()[offset] / MIB_SIZE,
      (memory.bar1Used.get()[offset] * 100) / memory.bar1Total.get());
  std::string memLine = fmt::format(
      "Memory: {}MiB ({}%)", memory.memoryUsed.get()[offset] / MIB_SIZE,
  ...
  if (ImPlot::BeginPlot("Memory", plotSize, s_plotFlags))
  {
    ...
    ImPlot::SetupAxes(nullptr, "Bytes",
                      s_axesFlags | ImPlotAxisFlags_NoDecorations, s_axesFlags);
    ImPlot::SetupAxesLimits(0, SAMPLING_NUM, 0,
                            static_cast<double>(memory.memoryTotal.get()));
    ...
    if (ImPlot::IsPlotHovered())
    {
      ...
      ImGui::Text("%s",
                  fmt::format("Used Memory: {}MiB",
                              memory.memoryUsed.get()[mouseOffset] / MIB_SIZE)
                      .c_str());
      ImGui::EndTooltip();
    }
  ...
  if (ImPlot::BeginPlot("BAR1", plotSize, s_plotFlags))
  {
    ...
    ImPlot::SetupAxes(nullptr, "Bytes",
                      s_axesFlags | ImPlotAxisFlags_NoDecorations, s_axesFlags);
    ImPlot::SetupAxesLimits(0, SAMPLING_NUM, 0,
                            static_cast<double>(memory.bar1Total.get()));
    ...
    if (ImPlot::IsPlotHovered())
    {
      ...
      ImGui::BeginTooltip();
      ImGui::Text("%s",
                  fmt::format("Used BAR1 Memory: {}MiB",
                              memory.bar1Used.get()[mouseOffset] / MIB_SIZE)
                      .c_str());
      ImGui::EndTooltip();
    }
...
void ElementGpuMonitor::imguiDevicePerformanceState(uint32_t deviceIndex)
{
  ...
  uint32_t generalMaxClock = std::max(
      deviceInfo.clockMaxGraphics.get(),
      std::max(deviceInfo.clockMaxSM.get(), deviceInfo.clockMaxVideo.get()));

  const int offset = m_nvmlMonitor->getOffset();

  std::string graphicsClockLine = fmt::format(
      "Graphics: {}MHz", performanceState.clockGraphics.get()[offset]);
  std::string smClockLine =
      fmt::format("SM: {}MHz", performanceState.clockSM.get()[offset]);
  std::string videoClockLine =
      fmt::format("Video: {}MHz", performanceState.clockVideo.get()[offset]);
                        ...
                        s_plotFlags))
  {
    ...
    if (ImPlot::IsPlotHovered())
    {
      ...
      ImGui::Text("Graphics: %dMHz",
                  performanceState.clockGraphics.get()[clockOffset]);
      ImGui::Text("SM: %dMHz", performanceState.clockSM.get()[clockOffset]);
      ImGui::Text("Video: %dMHz",
                  performanceState.clockVideo.get()[clockOffset]);
      ImGui::EndTooltip();
    }
  ...
  if (ImPlot::BeginPlot("Memory Clock", plotSize, s_plotFlags))
  {
    ...
    if (ImPlot::IsPlotHovered())
    {
      ...
      ImGui::Text("Memory: %dMHz",
                  performanceState.clockMem.get()[clockOffset]);
      ImGui::EndTooltip();
    }
  ...
  if (ImPlot::BeginPlot("Throttle reason", plotSize, s_plotFlags))
  {
    ...
    for (size_t i = 0; i < throttleValues.size(); i++)
    {
      ...
      throttleStrings[i] =
          NvmlMonitor::DevicePerformanceState::getThrottleReasonStrings(r)[0];
    ...
    ImPlot::SetupAxisTicks(ImAxis_Y1, throttleValues.data(),
                           static_cast<int>(throttleValues.size()),
...
void ElementGpuMonitor::imguiDevicePowerState(uint32_t deviceIndex)
{
  ...
  std::string temperatureLine =
      fmt::format("Temperature: {}C", powerState.temperature.get()[offset]);
  std::string powerLine =
      fmt::format("Power: {}W", powerState.power.get()[offset]);
  std::string fanSpeedLine =
      fmt::format("Fan speed: {}%", powerState.fanSpeed.get()[offset]);
  ...
  if (ImPlot::BeginPlot("Temperature", plotSize, s_plotFlags))
  {
    ...
    ImPlot::SetupAxes(nullptr, "Celsius",
                      s_axesFlags | ImPlotAxisFlags_NoDecorations, s_axesFlags);
    ImPlot::SetupAxesLimits(0, SAMPLING_NUM, 0,
                            info.tempThresholdShutdown.get());
...
void ElementGpuMonitor::imguiDeviceUtilization(uint32_t deviceIndex)
{
  ...
  std::string gpuUtilizationLine =
      fmt::format("GPU: {}%", utilization.gpuUtilization.get()[offset]);
  std::string memUtilizationLine =
      fmt::format("Memory: {}%", utilization.memUtilization.get()[offset]);

  std::string graphicsProcessLine = fmt::format(
      "Graphics processes: {}", utilization.graphicsProcesses.get()[offset]);
  std::string computeProcessLine = fmt::format(
      "Compute processes: {}", utilization.computeProcesses.get()[offset]);
  ...
  if (ImPlot::BeginPlot("GPU and Memory Utilization", plotSize, s_plotFlags))
  {
    ...
    if (ImPlot::IsPlotHovered())
    {
      ...
      ImGui::Text("Memory: %d%%",
                  utilization.memUtilization.get()[mouseOffset]);
      ImGui::EndTooltip();
    }
                        ...
                        s_plotFlags))
  {
    ...
    if (ImPlot::IsPlotHovered())
    {
      ...
      ImGui::Text("Graphics: %d",
                  utilization.graphicsProcesses.get()[mouseOffset]);
      ImGui::Text("Compute: %d",
                  utilization.computeProcesses.get()[mouseOffset]);
      ImGui::EndTooltip();
    }
...
void ElementGpuMonitor::imguiClockSetup(uint32_t deviceIndex)
{
      ...
      !deviceInfo.supportedGraphicsClocks.get().empty())
  {
    ...
    if (ImGui::BeginCombo(
            fmt::format("###DevSupportedGraphicsClocksMemCombo{}", deviceIndex)
                .c_str(),
            fmt::format(
                "{}MHz",
    ...
    uint32_t currentSelectedMemClock =
        deviceInfo.supportedMemoryClocks.get()[m_selectedMemClock];
    uint32_t currentSelectedGfxClock = deviceInfo.supportedGraphicsClocks.get()
                                           .find(activeMemClock)
                                           ->second[m_selectedGraphicsClock];

    std::string nvidiaSmiMemClockLockCommand =
        fmt::format("nvidia-smi -i {} -lmc {},{}", deviceIndex,
                    currentSelectedMemClock, currentSelectedMemClock);
    std::string nvidiaSmiGfxClockLockCommand =
        fmt::format("nvidia-smi -i {} -lgc {},{}", deviceIndex,
                    currentSelectedGfxClock, currentSelectedGfxClock);

    ImGui::Text("NVIDIA-SMI Commands");
    ImGui::TreePush("NVIDIA-SMI Commands");

    if (ImGui::BeginTable(
            fmt::format("NVIDIA-SMI commands###NVSMICMD{}", deviceIndex)
                .c_str(),
            2,
            ImGuiTableFlags_Borders | ImGuiTableFlags_HighlightHoveredColumn |
                ImGuiTableFlags_RowBg))
    {
      ...
      ImGui::TableNextColumn();
      imguiCopyableText(
          nvidiaSmiMemClockLockCommand,
          reinterpret_cast<uint64_t>(&nvidiaSmiMemClockLockCommand));
      ...
      ImGui::TableNextColumn();
      imguiCopyableText(
          nvidiaSmiGfxClockLockCommand,
          reinterpret_cast<uint64_t>(&nvidiaSmiGfxClockLockCommand));

      ImGui::TableNextRow();
      ImGui::TableNextColumn();
      ImGui::Text("Memory clock unlock (reset to default behavior)");
      ImGui::TableNextColumn();
      std::string memoryClockResetCommand =
          fmt::format("nvidia-smi -i {} -rmc", deviceIndex);
      imguiCopyableText(memoryClockResetCommand,
                        reinterpret_cast<uint64_t>(&memoryClockResetCommand));

      ImGui::TableNextRow();
      ImGui::TableNextColumn();
      ImGui::Text("Graphics clock unlock (reset to default behavior)");
      ImGui::TableNextColumn();
      std::string graphicsClockResetCommand =
          fmt::format("nvidia-smi -i {} -rgc", deviceIndex);
      imguiCopyableText(graphicsClockResetCommand,
                        reinterpret_cast<uint64_t>(&graphicsClockResetCommand));
