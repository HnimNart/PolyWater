 ...
 * AFFILIATES. SPDX-License-Identifier: LicenseRef-NvidiaProprietary
 *
...
void NvmlMonitor::DeviceInfo::refresh(void *dev) {
  ...
  CHECK_NVML_SUPPORT(nvmlDeviceGetBoardPartNumber(
                         device, partNumber.get().data(),
                         static_cast<uint32_t>(partNumber.get().size())),
                     partNumber);
  ...
  for (int i = 0; i < bridgeChipHierarchy.bridgeCount; i++) {
    bridgeHierarchy.get()[i].first =
        ((bridgeChipHierarchy.bridgeChipInfo[i].type == NVML_BRIDGE_CHIP_PLX)
             ? "PLX"
             : "BRO4");
    bridgeHierarchy.get()[i].second =
        fmt::format("#{}", bridgeChipHierarchy.bridgeChipInfo[i].fwVersion);
  }
  ...
  CHECK_NVML_SUPPORT(
      nvmlDeviceGetCudaComputeCapability(device, &computeCapabilityMajor.get(),
                                         &computeCapabilityMinor.get()),
      computeCapabilityMajor);
  computeCapabilityMinor.isSupported = computeCapabilityMajor.isSupported;

  CHECK_NVML_SUPPORT(
      nvmlDeviceGetCurrPcieLinkGeneration(device, &pcieLinkGen.get()),
      pcieLinkGen);
  CHECK_NVML_SUPPORT(
      nvmlDeviceGetCurrPcieLinkWidth(device, &pcieLinkWidth.get()),
      pcieLinkWidth);

  CHECK_NVML_SUPPORT(
      nvmlDeviceGetDefaultApplicationsClock(device, NVML_CLOCK_GRAPHICS,
                                            &clockDefaultGraphics.get()),
      clockDefaultGraphics);
  CHECK_NVML_SUPPORT(nvmlDeviceGetMaxClockInfo(device, NVML_CLOCK_GRAPHICS,
                                               &clockMaxGraphics.get()),
                     clockMaxGraphics);
  CHECK_NVML_SUPPORT(
      nvmlDeviceGetMaxCustomerBoostClock(device, NVML_CLOCK_GRAPHICS,
                                         &clockBoostGraphics.get()),
      clockBoostGraphics);

  CHECK_NVML_SUPPORT(nvmlDeviceGetDefaultApplicationsClock(
                         device, NVML_CLOCK_SM, &clockDefaultSM.get()),
                     clockDefaultSM);
  CHECK_NVML_SUPPORT(
      nvmlDeviceGetMaxClockInfo(device, NVML_CLOCK_SM, &clockMaxSM.get()),
      clockMaxSM);
  CHECK_NVML_SUPPORT(nvmlDeviceGetMaxCustomerBoostClock(device, NVML_CLOCK_SM,
                                                        &clockBoostSM.get()),
                     clockBoostSM);

  CHECK_NVML_SUPPORT(nvmlDeviceGetDefaultApplicationsClock(
                         device, NVML_CLOCK_MEM, &clockDefaultMem.get()),
                     clockDefaultMem);
  CHECK_NVML_SUPPORT(
      nvmlDeviceGetMaxClockInfo(device, NVML_CLOCK_MEM, &clockMaxMem.get()),
      clockMaxMem);
  CHECK_NVML_SUPPORT(nvmlDeviceGetMaxCustomerBoostClock(device, NVML_CLOCK_MEM,
                                                        &clockBoostMem.get()),
                     clockBoostMem);

  CHECK_NVML_SUPPORT(nvmlDeviceGetDefaultApplicationsClock(
                         device, NVML_CLOCK_VIDEO, &clockDefaultVideo.get()),
                     clockDefaultVideo);
  CHECK_NVML_SUPPORT(
      nvmlDeviceGetMaxClockInfo(device, NVML_CLOCK_VIDEO, &clockMaxVideo.get()),
      clockMaxVideo);
  CHECK_NVML_SUPPORT(nvmlDeviceGetMaxCustomerBoostClock(
  ...
  CHECK_NVML_SUPPORT(nvmlDeviceGetEncoderCapacity(device,
                                                  NVML_ENCODER_QUERY_H264,
                                                  &encoderCapacityH264.get()),
                     encoderCapacityH264);
  CHECK_NVML_SUPPORT(nvmlDeviceGetEncoderCapacity(device,
                                                  NVML_ENCODER_QUERY_HEVC,
                                                  &encoderCapacityHEVC.get()),
                     encoderCapacityHEVC);

  infoROMImageVersion.get().resize(NVML_DEVICE_INFOROM_VERSION_BUFFER_SIZE);
  CHECK_NVML_SUPPORT(
      nvmlDeviceGetInforomImageVersion(
          device, infoROMImageVersion.get().data(),
  ...
  CHECK_NVML_SUPPORT(nvmlDeviceGetInforomVersion(
                         device, NVML_INFOROM_OEM,
                         infoROMOEMVersion.get().data(),
                         static_cast<uint32_t>(infoROMOEMVersion.get().size())),
                     infoROMOEMVersion);
  CHECK_NVML_SUPPORT(nvmlDeviceGetInforomVersion(
                         device, NVML_INFOROM_ECC,
                         infoROMECCVersion.get().data(),
                         static_cast<uint32_t>(infoROMECCVersion.get().size())),
                     infoROMECCVersion);
  CHECK_NVML_SUPPORT(
      nvmlDeviceGetInforomVersion(
          device, NVML_INFOROM_POWER, infoROMPowerVersion.get().data(),
          static_cast<uint32_t>(infoROMPowerVersion.get().size())),
      infoROMPowerVersion);

  CHECK_NVML_SUPPORT(
      nvmlDeviceGetMaxPcieLinkGeneration(device, &maxLinkGen.get()),
      maxLinkGen);
  CHECK_NVML_SUPPORT(nvmlDeviceGetMaxPcieLinkWidth(device, &maxLinkWidth.get()),
                     maxLinkWidth);
  CHECK_NVML_SUPPORT(nvmlDeviceGetMinorNumber(device, &minorNumber.get()),
                     minorNumber);
  CHECK_NVML_SUPPORT(nvmlDeviceGetMultiGpuBoard(device, &multiGpuBool.get()),
                     multiGpuBool);
  deviceName.get().resize(NVML_DEVICE_NAME_V2_BUFFER_SIZE);
  CHECK_NVML_SUPPORT(
      nvmlDeviceGetName(device, deviceName.get().data(),
                        static_cast<uint32_t>(deviceName.get().size())),
      deviceName);
  ...
  CHECK_NVML_SUPPORT(nvmlDeviceGetVbiosVersion(
                         device, vbiosVersion.get().data(),
                         static_cast<uint32_t>(vbiosVersion.get().size())),
                     vbiosVersion);

  CHECK_NVML_SUPPORT(nvmlDeviceGetTemperatureThreshold(
  ...
  CHECK_NVML_SUPPORT(nvmlDeviceGetTemperatureThreshold(
                         device, NVML_TEMPERATURE_THRESHOLD_MEM_MAX,
  ...
  CHECK_NVML_SUPPORT(
      nvmlDeviceGetPowerManagementLimit(device, &powerLimit.get()), powerLimit);
  // Milliwatt to watt
  powerLimit.get() /= 1000;
...
void NvmlMonitor::DeviceMemory::refresh(void *dev, uint32_t offset) {
  ...
  nvmlMemory_t memory{};
  CHECK_NVML_SUPPORT(nvmlDeviceGetBAR1MemoryInfo(device, &bar1Memory),
                     bar1Total);
...
void NvmlMonitor::DeviceUtilization::refresh(void *dev, uint32_t offset) {
  ...
  nvmlUtilization_t utilization;
  CHECK_NVML_SUPPORT(nvmlDeviceGetUtilizationRates(device, &utilization),
                     gpuUtilization);
  ...
  CHECK_NVML_SUPPORT(nvmlDeviceGetComputeRunningProcesses(
                         device, &computeProcesses.get()[offset], nullptr),
                     computeProcesses);
  CHECK_NVML_SUPPORT(nvmlDeviceGetGraphicsRunningProcesses(
                         device, &graphicsProcesses.get()[offset], nullptr),
                     graphicsProcesses);
#endif
}
...
void NvmlMonitor::DevicePerformanceState::refresh(void *dev, uint32_t offset) {
  ...
  CHECK_NVML_SUPPORT(nvmlDeviceGetClockInfo(device, NVML_CLOCK_GRAPHICS,
                                            &clockGraphics.get()[offset]),
                     clockGraphics);
  CHECK_NVML_SUPPORT(
      nvmlDeviceGetClockInfo(device, NVML_CLOCK_SM, &clockSM.get()[offset]),
      clockSM);
  CHECK_NVML_SUPPORT(
      nvmlDeviceGetClockInfo(device, NVML_CLOCK_MEM, &clockMem.get()[offset]),
      clockMem);
  CHECK_NVML_SUPPORT(nvmlDeviceGetClockInfo(device, NVML_CLOCK_VIDEO,
                                            &clockVideo.get()[offset]),
                     clockVideo);
...
void NvmlMonitor::DevicePowerState::refresh(void *dev, uint32_t offset) {
  ...
  CHECK_NVML_SUPPORT(nvmlDeviceGetTemperature(device, NVML_TEMPERATURE_GPU,
                                              &temperature.get()[offset]),
                     temperature);
  CHECK_NVML_SUPPORT(nvmlDeviceGetPowerUsage(device, &power.get()[offset]),
                     power);
  // Milliwatt to watt
  power.get()[offset] /= 1000;
  CHECK_NVML_SUPPORT(nvmlDeviceGetFanSpeed(device, &fanSpeed.get()[offset]),
                     fanSpeed);
#endif
}
