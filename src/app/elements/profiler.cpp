  ...
  for (auto &view : m_views) {
    ...
    if (ImGui::Begin(view.state->name.c_str(), &view.state->show)) {
      ...
      if (ImGui::BeginTabBar("Profiler Tabs")) {
        if (ImGui::BeginTabItem("Table", NULL,
                                view.selectDefaultTab &&
                                        view.state->defaultTab == TABLE
        ...
        if (ImGui::BeginTabItem("BarChart", NULL,
                                view.selectDefaultTab &&
                                        view.state->defaultTab == BAR_CHART
                                    ? ImGuiTabItemFlags_SetSelected
                                    : 0)) {
          renderBarChart(view);
          ImGui::EndTabItem();
        ...
        if (ImGui::BeginTabItem("LineChart", NULL,
                                view.selectDefaultTab &&
                                        view.state->defaultTab == LINE_CHART
                                    ? ImGuiTabItemFlags_SetSelected
                                    : 0)) {
        ...
        if (ImGui::BeginTabItem("PieChart", NULL,
                                view.selectDefaultTab &&
                                        view.state->defaultTab == PIE_CHART
                                    ? ImGuiTabItemFlags_SetSelected
                                    : 0)) {
          renderPieChart(view);
          ImGui::EndTabItem();
...
void core::ElementProfiler::updateData() {
  ...
  for (const auto &snapshot : m_frameSnapshots) {
    ...
    addEntries(snapshot, m_frameNodes.back().child, 0,
               (uint32_t)snapshot.timerInfos.size());
  }
  for (const auto &snapshot : m_singleSnapshots) {
    EntryNode entryNode;
    entryNode.name = snapshot.name;
    m_singleNodes.emplace_back(entryNode);
    addEntries(snapshot, m_singleNodes.back().child, 0,
               (uint32_t)snapshot.timerInfos.size());
  }
...
uint32_t core::ElementProfiler::addEntries(
  ...
  for (uint32_t curIndex = startIndex; curIndex < endIndex; curIndex++) {
    ...
    uint32_t nextLevel = curIndex + 1 < endIndex
                             ? snapshot.timerInfos[curIndex + 1].level
                             : currentLevel;
    if (nextLevel > currentLevel) {
      curIndex = addEntries(snapshot, entryNode.child, curIndex + 1, endIndex,
                            nextLevel);
    }
    nodes.emplace_back(entryNode);
    if (nextLevel < currentLevel)
...
void core::ElementProfiler::displayTableNode(const EntryNode &node,
  ...
  flags =
      is_folder
          ? flags |
                (depth < defaultOpenLevels ? ImGuiTreeNodeFlags_DefaultOpen : 0)
  ...
  if ((open) && is_folder) {
    for (int child_n = 0; child_n < static_cast<int>(node.child.size());
         child_n++) {
      displayTableNode(node.child[child_n], detailed, defaultOpenLevels,
                       depth + 1);
    }
    ImGui::TreePop();
  }
...
void core::ElementProfiler::renderTable(View &view) {
  ...
  const int minGridSize =
      view.state->table.detailed
          ? 1500
          : 550; // minimum size of container for responsive grid mode
  const bool gridMode = ImGui::GetContentRegionAvail().x >= minGridSize &&
                        m_frameNodes.size() > 1;
  const float width =
      ImGui::GetContentRegionAvail().x / (gridMode ? 2.0f : 1.0f) - 5.0f;
  ...
  for (size_t i = 0; i < m_frameNodes.size(); ++i) {
    if (i > 0) {
      if (gridMode)
        ...
        ImGui::Spacing(); // add a small vertical gap between tables, for better
                          // legibility
    }
    ...
    if (!m_frameNodes[i].child.empty() || m_singleNodes[i].child.empty()) {
      ...
      if (ImGui::BeginTable("EntryTable", colCount, tableFlags,
                            ImVec2(width, 0))) {
        ImGui::TableSetupColumn(m_frameNodes[i].name.c_str(),
        ...
        if (view.state->table.detailed) {
          ...
          ImGui::TableSetupColumn("GPU min",
                                  ImGuiTableColumnFlags_WidthStretch);
...
void core::ElementProfiler::renderPieChart(View &view) {
  ...
  const float chartWidth = (ImGui::GetContentRegionAvail().x - legendWidth) /
                               (gridMode ? 2.0f : 1.0f) -
                           5.0f;
  ...
  if (ImGui::IsItemHovered(ImGuiHoveredFlags_DelayShort))
    ImGui::SetTooltip(
        "Use CPU frame time as total time, otherwise use sum of GPU timers.");
  ...
  for (auto i = 0; i < m_frameNodes.size(); ++i) {
    ...
    if (!rootNode.child.empty()) {
      ...
      if (ImPlot::BeginPlot(rootNode.name.c_str(),
                            ImVec2(width, (float)view.state->plotHeight),
        ...
        const float aspectRatio =
            0.5f * (float)chartWidth / (float)view.state->plotHeight;
        ImPlot::SetupAxesLimits(0.5 - aspectRatio, 0.5 + aspectRatio, 0, 1,
                                ImPlotCond_Always);

        const double totalTime =
            (view.state->pieChart.cpuTotal ? node.cpuTime : node.gpuTime);
...
void core::ElementProfiler::renderPieChartNode(const EntryNode &node, int level,
  ...
  const double myRadius =
      numLevels == 1 ? plotRadius
                     : plotRadius * (1.0 - (0.5 * level) / (numLevels - 1));
  ...
  ImPlot::PlotPieChart(labels.data(), data.data(),
                       static_cast<int>(data.size()), 0.5, 0.5, myRadius, text,
                       angle0);

  // Recurse over children
  if (level + 1 < numLevels && node.child.size() > 0) {
    for (size_t i = 0; i < node.child.size(); i++) {
      renderPieChartNode(node.child[i], level + 1, numLevels, plotRadius,
                         angle0, totalTime);
...
void core::ElementProfiler::renderBarChart(View &view) {
  ...
  ImGui::SameLine();
  ImGui::CheckboxFlags("Stacked", (unsigned int *)&view.state->barChart.stacked,
    ...
    if (!rootNode.child.empty()) {
      ...
      if (ImPlot::BeginPlot(rootNode.name.c_str(),
        ...
        ImPlot::SetupLegend(ImPlotLocation_NorthEast,
                            ImPlotLegendFlags_Outside);
        ImPlot::SetupAxes(
            "Time in milliseconds", "Timers", ImPlotAxisFlags_AutoFit,
        ...
        if (items > 0) {
          ImPlot::PlotBarGroups(
              labels1.data(), data1.data(), items, groups, size, 0,
  ...
  for (auto i = 0; i < m_frameNodes.size(); ++i) {
    ...
    if (!rootNode.child.empty()) {
      ...
      if (view.maxY == 0.f) {
        ...
        view.maxY = (PROFILER_GRAPH_TEMPORAL_SMOOTHING * view.maxY + avgTime) /
                    (PROFILER_GRAPH_TEMPORAL_SMOOTHING + 1.f);
      }
      ...
      if (gpuTimes.size() > 0 && gpuTimes[0].size() > 0) {
        ...
        const ImPlotFlags plotFlags = ImPlotFlags_NoBoxSelect |
                                      ImPlotFlags_NoMouseText |
        ...
        if (ImPlot::BeginPlot(rootNode.name.c_str(),
          ...
          ImPlot::SetupAxes(nullptr, "Count",
                            axesFlags | ImPlotAxisFlags_NoTickLabels,
                            axesFlags);
          ImPlot::SetupAxesLimits(0, node.child[0].timerInfo.numAveraged, 0,
                                  view.maxY * 1.2, ImPlotCond_Always);
          ...
          for (size_t i = 0; i < node.child.size(); i++) {
            ...
            if (view.state->lineChart.gpuFills) {
              ...
              ImPlot::PlotShaded(
                  node.child[index].name.c_str(), gpuTimes[index].data(),
                  (int)gpuTimes[index].size(), -INFINITY, 1.0, 0, 0, 0);
              ImPlot::PopStyleVar();
            }
            if (view.state->lineChart.gpuLines) {
              ImPlot::PlotLine(node.child[index].name.c_str(),
                               gpuTimes[index].data(),
                               (int)gpuTimes[index].size());
            }
          }
          ...
          if (ImPlot::IsPlotHovered()) {
            ...
            for (size_t i = 0; i < node.child.size(); i++) {
              if (i == 0) {
                ...
                localTimes[i] =
                    gpuTimes[i][mouseOffset] - gpuTimes[i - 1][mouseOffset];
              }
              totalGpu += localTimes[i];
            }
            ImGui::Text("GPU: %.3f ms", totalGpu);
            for (size_t i = 0; i < node.child.size(); i++) {
              ImGui::Text("  %s: %.3f ms (%.1f%%)", node.child[i].name.c_str(),
                          localTimes[i], localTimes[i] * 100.f / totalGpu);
            }
...
void core::ElementProfiler::addSettingsHandler() {
  ...
  // Save settings handler, not using capture so can be used as a function
  // pointer
  ...
  // Load settings handler, not using capture so can be used as a function
  // pointer
