...
/**********************************************************/
{
  ...
  if (PE::begin())
  {
    ...
    changed |= PE::SliderFloat("Exposure", &tonemapper.exposure, 0.1F, 200.0F,
                               "%.3f", ImGuiSliderFlags_Logarithmic,
                               "Multiplier for input colors (0.1 = very dark, "
                               "1 = neutral, 200 = very bright)");
    ...
    const float resetButtonWidth =
        ImGui::CalcTextSize(ICON_MS_RESET_WHITE_BALANCE).x +
        ImGui::GetStyle().FramePadding.x * 2.F;
    const float whiteBalanceSliderWidth =
    ...
    PE::entry(
        ...
        "(6506K = D65 neutral, higher values make the image more orange "
        "because they're correcting for cooler lighting)");
    ...
    PE::entry(
        ...
        "Green/magenta lighting tint to correct for in ANSI C78.377-2008 Duv "
        "units "
        "(-.03 = very green, 0 = blackbody, .00326 = D65 neutral, .03 = very "
        "magenta)");

    changed |=
        PE::SliderFloat("Contrast", &tonemapper.contrast, 0.0F, 2.0F, "%.2f", 0,
                        "Scales colors away from gray (0 = no contrast, 1 = "
                        "neutral, 2 = high contrast)");
    changed |= PE::SliderFloat("Brightness", &tonemapper.brightness, 0.0F, 2.0F,
                               "%.2f", 0,
                               "Gamma curve for output colors (1 = neutral, "
                               "higher values make midtones brighter)");
    changed |= PE::SliderFloat("Saturation", &tonemapper.saturation, 0.0F, 2.0F,
                               "%.2f", 0,
                               "Controls color intensity (0 = grayscale, 1 = "
                               "neutral, 2 = high saturation)");
    changed |= PE::SliderFloat(
        "Vignette", &tonemapper.vignette, -1.0F, 1.0F, "%.2f", 0,
    ...
    if (tonemapper.autoExposure)
    {
      ...
      changed |= PE::Combo("Average Mode", (int*) &tonemapper.averageMode,
                           ...
                           "darker and 50% of pixels are brighter)");

      changed |= PE::DragFloat("Adaptation Speed",
                               &tonemapper.autoExposureSpeed, 0.001f, 0.f,
                               100.f, "%.3f", ImGuiSliderFlags_AlwaysClamp,
                               "How quickly auto exposure adapts to lighting "
                               "changes (higher = faster adaptation)");
      changed |= PE::DragFloat("Min (EV100)", &tonemapper.evMinValue, 0.01f,
                               -24.f, 24.f, "%.2f", 0,
                               "Minimum histogram luminance in logarithmic "
                               "stops (-24 = very dark, +24 = very bright)");
      changed |= PE::DragFloat("Max (EV100)", &tonemapper.evMaxValue, 0.01f,
                               -24.f, 24.f, "%.2f", 0,
                               "Maximum histogram luminance in logarithmic "
                               "stops (-24 = very dark, +24 = very bright)");
      ...
      changed |=
          PE::DragFloat("Center Metering Size", &tonemapper.centerMeteringSize,
                        0.01f, 0.01f, 1.0f, "%.2f", 0,
                        "Size of center area for exposure calculation (0.01 = "
                        "small spot, 1.0 = full frame)");
      ImGui::EndDisabled();
    }
    changed |=
