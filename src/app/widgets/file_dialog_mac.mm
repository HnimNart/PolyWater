...
static NSArray<NSString *> *ParseExtensions(const char *exts) {
  if (!exts || strlen(exts) == 0 || strcmp(exts, "*") == 0 || strcmp(exts, "*.*") == 0) {
    return nil;
  }
  ...
  while (start < s.size()) {
    ...
    start = sep + 1;
    sep = s.find('|', start);
    std::string spec = (sep == std::string::npos) ? s.substr(start) : s.substr(start, sep - start);
    
    // Process the spec which might be "*.gltf;*.glb"
    size_t spec_start = 0;
    while (spec_start < spec.size()) {
        size_t semi = spec.find(';', spec_start);
        std::string ext = (semi == std::string::npos) ? spec.substr(spec_start) : spec.substr(spec_start, semi - spec_start);
        
        // Clean "*.ext" to "ext"
        if (ext.starts_with("*.")) ext = ext.substr(2);
        if (ext != "*" && ext != "*.*") {
...
RunDialog(bool isSave, bool isFolder, const char *title, const char *exts,
  ...
  @autoreleasepool {
    ...
    if (initialDir && !initialDir->empty() && std::filesystem::exists(*initialDir)) {
       NSString* pathStr = [NSString stringWithUTF8String:initialDir->string().c_str()];
...
std::filesystem::path windowOpenFileDialog(struct GLFWwindow *glfwin, const char *title, const char *exts,
  ...
  std::filesystem::path result = RunDialog(false, false, title, exts, &initialDir);
  if (!result.empty()) {
