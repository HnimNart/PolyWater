...
// - The first camera in the list is the HOME camera, the one that was set
// before this is called.
// - The update function will check if something has changed and will save the
// JSON to disk, only
//  once in a while.
// - Adding a camera will be added only if it is different from all other saved
// cameras
...
struct CameraPresetManager
{
  ...
  void loadSetting(std::shared_ptr<core::CameraManipulator> cameraM)
  {
    ...
    try
    {
      ...
      if (getJsonValue(j, "mode", iVal))
        cameraM->setMode(static_cast<core::CameraManipulator::Modes>(iVal));
  ...
  std::vector<core::CameraManipulator::Camera> m_cameras{};
...
/**********************************************************/
{
  ...
  for (int n = 1; n < buttonsCount; n++)
  {
    ...
    if (ImGui::IsItemHovered() &&
        ImGui::GetIO().MouseClicked[ImGuiMouseButton_Middle])
      delete_item = n;
    ...
    float next_button_x2 = last_button_x2 + s_buttonSpacing +
                           ImGui::CalcTextSize(nextLabel.c_str()).x +
                           ImGui::GetStyle().FramePadding.x * 2.f;
...
/**********************************************************/
{
  ...
  ImGui::Separator();
  // Dear ImGui in v1.92 has a FIXME where it doesn't add 1px of spacing after
  // separators
  ...
  if (showSettings)
  {
    if (PeBeginAutostretch("##Speed"))
    {
      ...
      changed |= PE::DragFloat(
          "Speed", &speed, 2e-4f * (speedMax - speedMin), speedMin, speedMax,
          "%.2f", ImGuiSliderFlags_Logarithmic, "Speed of camera movement");
...
/**********************************************************/
{
  ...
  if (ImGui::TreeNodeEx("Projection", flag))
  {
    if (PeBeginAutostretch("##Projection"))
    {
      ...
      changed |=
          PE::DragFloat2("Z-Clip", &camera.clip.x, 2e-5f * (maxClip - minClip),
                         minClip, maxClip, "%.6f", ImGuiSliderFlags_Logarithmic,
                         "Near/Far clip planes for depth buffer");
...
/**********************************************************/
{
  ...
  core::CameraManipulator::Camera camera = cameraManip->getCamera();
  ...
  // Main camera panel with modern design
  {
    ...
    instantChanged |=
        ProjectionSettingsSection(cameraManip, camera,
                                  (openSections & CameraSection_Projection)
                                      ? ImGuiTreeNodeFlags_DefaultOpen
                                      : ImGuiTreeNodeFlags_None);

    changed |= PositionSection(cameraManip, camera,
                               (openSections & CameraSection_Position)
                                   ? ImGuiTreeNodeFlags_DefaultOpen
                                   : ImGuiTreeNodeFlags_None);

    // Up vector / Animation duration section
    changed |= OtherSettingsSection(cameraManip, camera,
                                    (openSections & CameraSection_Other)
                                        ? ImGuiTreeNodeFlags_DefaultOpen
                                        : ImGuiTreeNodeFlags_None);
