...
// This is a helper class to manage settings in ImGui. It allows to easily add
// settings to the ImGui settings handler
...
void app::SettingsHandler::setSetting(const std::string &key, T *value) {
  ...
  if constexpr (std::is_same_v<T, glm::ivec2> ||
                std::is_same_v<T, glm::uvec2> || std::is_same_v<T, glm::vec2>) {
