...
public:
  ...
  void setup_scene(VkCommandBuffer cmd) {
    ...
    tinygltf::Model teapotModel = scene_resources.loadGltf(
        nvutils::findFile("teapot.gltf", nvsamples::getResourcesDirs()));
    tinygltf::Model planeModel = scene_resources.loadGltf(
        nvutils::findFile("plane.gltf", nvsamples::getResourcesDirs()));
    ...
    // Teapot material
    SceneResources::MaterialID teapot_id = scene_resources.addMaterial(
        {.baseColorFactor = glm::vec4(0.8f, 1.0f, 0.6f, 1.0f),
         .metallicFactor = 0.5f,
         .roughnessFactor = 0.5f});
    // Plane material with texture
    SceneResources::MaterialID plane_id = scene_resources.addMaterial(
    ...
    nvsamples::GltfSceneResource &gltf_resources =
        m_scene_manager->gltf_resources();
    ...
    sceneInfo.meshes =
        (shaderio::GltfMesh *)
            gltf_resources.bMeshes.address; // Address of the mesh buffer
    sceneInfo.materials =
    ...
    m_cameraManip->setLookat({0.0F, 0.5F, 5.0}, {0.F, 0.F, 0.F},
                             {0.0F, 1.0F, 0.0F});
  }
  ...
  void onAttach(nvapp::Application *app) override {
    ...
    ctx = VulkanContext::create(*m_ctx, descriptorPool,
                                m_app->getViewportSize(), compiler);
    // m_vulkan_scene_renderer = std::make_shared<VulkanSceneRenderer>(ctx);
    // m_scene_manager =
    // std::make_unique<SceneManager>(m_vulkan_scene_renderer);
    // m_scene_manager->set_camera(m_cameraManip);

    // Create scene
    VkCommandBuffer cmd = m_app->createTempCmdBuffer();
    setup_scene(cmd);
    m_app->submitAndWaitTempCmdBuffer(
        cmd); // Submit the command buffer to upload the resources
  ...
  //---------------------------------------------------------------------------------------------------------------
  // Rendering all UI elements, this includes the image of the GBuffer, the
  // camera controls, and the sky parameters.
  // - Called every frame
  void onUIRender() override {
    ...
    if (ImGui::Begin("Viewport")) {
      ImGui::Image(
          ImTextureID(m_vulkan_scene_renderer->gbuffers().getDescriptorSet(
              eImgTonemapped)),
    ...
    if (ImGui::Begin("Settings")) {
      ...
      if (ImGui::CollapsingHeader("Environment")) {
        ImGui::Checkbox(
            "Use Sky",
            (bool *)&m_scene_manager->gltf_resources().sceneInfo.useSky);
        if (m_scene_manager->gltf_resources().sceneInfo.useSky)
          nvgui::skySimpleParametersUI(
              m_scene_manager->gltf_resources().sceneInfo.skySimpleParam);
        else {
          PE::begin();
          PE::ColorEdit3("Background",
                         (float *)&m_scene_manager->gltf_resources()
                             .sceneInfo.backgroundColor);
          PE::end();
          // Light
          PE::begin();
          if (m_scene_manager->gltf_resources()
                      .sceneInfo.punctualLights[0]
                      .type == shaderio::GltfLightType::ePoint ||
              m_scene_manager->gltf_resources()
                      .sceneInfo.punctualLights[0]
                      .type == shaderio::GltfLightType::eSpot) {
            PE::DragFloat3("Light Position",
                           glm::value_ptr(m_scene_manager->gltf_resources()
                                              .sceneInfo.punctualLights[0]
                                              .position),
          ...
          if (m_scene_manager->gltf_resources()
                      .sceneInfo.punctualLights[0]
                      .type == shaderio::GltfLightType::eDirectional ||
              m_scene_manager->gltf_resources()
                      .sceneInfo.punctualLights[0]
                      .type == shaderio::GltfLightType::eSpot) {
          ...
          PE::Combo("Light Type",
                    (int *)&m_scene_manager->gltf_resources()
                        .sceneInfo.punctualLights[0]
                        .type,
      ...
      PE::SliderFloat2("Metallic/Roughness Override",
                       glm::value_ptr(m_scene_manager->metallic_roughness()),
  ...
  //---------------------------------------------------------------------------------------------------------------
  // Rendering the scene
  // The scene is rendered to a GBuffer and the GBuffer is displayed in the
  // ImGui window. Only the ImGui is rendered to the swapchain image.
  // - Called every frame
  void onRender(VkCommandBuffer cmd) override {
    NVVK_DBG_SCOPE(cmd); // <-- Helps to debug in NSight

    // Update the scene information buffer, this cannot be done in between
    // dynamic rendering
  ...
  void updateSceneBuffer(VkCommandBuffer cmd) {
    ...
    const glm::mat4 &projMatrix =
        m_scene_manager->camera()->getPerspectiveMatrix();
    ...
    nvvk::cmdBufferMemoryBarrier(
        cmd, {m_scene_manager->gltf_resources().bSceneInfo.buffer,
              VK_PIPELINE_STAGE_2_FRAGMENT_SHADER_BIT,
              VK_PIPELINE_STAGE_2_TRANSFER_BIT});
    vkCmdUpdateBuffer(cmd, m_scene_manager->gltf_resources().bSceneInfo.buffer,
                      0, sizeof(shaderio::GltfSceneInfo),
  ...
  void onLastHeadlessFrame() override {
    m_app->saveImageToFile(
        m_scene_manager->get_image(eImgTonemapped),
...
private:
  ...
  nvvk::ResourceAllocator
      m_allocator{}; // Resource allocator for Vulkan resources, used for
                     // buffers and images
  nvvk::StagingUploader
      m_stagingUploader{}; // Utility to upload data to the GPU
  ...
  bool m_useRayTracing =
      true; // Set to true to use ray tracing, false for rasterization
};
...
int main(int argc, char **argv) {
  ...
  auto elemCamera =
      std::make_shared<nvapp::ElementCamera>(); // Element to control the camera
                                                // movement
  auto windowTitle = std::make_shared<
      nvapp::ElementDefaultWindowTitle>(); // Element displaying the window
                                           // title with application name and
                                           // size
  auto windowMenu =
      std::make_shared<nvapp::ElementDefaultMenu>(); // Element displaying a
                                                     // menu, File->Exit ...
