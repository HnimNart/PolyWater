...
int main(int argc, char** argv)
{
  ...
  for (uint32_t ext_id = 0; ext_id < count;
       ext_id++)  // Adding required extensions (surface, win32, linux, ..)
    contextInfo.addInstanceExtension(reqExtensions[ext_id]);
  contextInfo.addInstanceLayer("VK_LAYER_LUNARG_monitor", true);              // FPS in titlebar
  contextInfo.addInstanceExtension(VK_EXT_DEBUG_UTILS_EXTENSION_NAME, true);  // Allow debug names
  contextInfo.addDeviceExtension(
      VK_KHR_SWAPCHAIN_EXTENSION_NAME);  // Enabling ability to present rendering
  ...
  helloVk.loadModel(nvh::findFile("assets/scenes/Medieval_building.obj", defaultSearchPaths, true));
  helloVk.loadModel(nvh::findFile("assets/scenes/plane.obj", defaultSearchPaths, true));
  helloVk.loadModel(nvh::findFile("assets/scenes/wuson.obj", defaultSearchPaths, true),
                    glm::scale(glm::mat4(1.f), glm::vec3(0.5f)) *
                        glm::translate(glm::mat4(1.f), glm::vec3(0.0f, 0.0f, 6.0f)));
