...
void HelloVulkan::updateUniformBuffer(const VkCommandBuffer &cmdBuf) {
  ...
  glm::mat4 proj = glm::perspectiveRH_ZO(glm::radians(CameraManip.getFov()),
...
void HelloVulkan::createDescriptorSetLayout() {
  ...
  m_descSetLayoutBind.addBinding(eImplicits, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
                                 1,
                                 VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR |
...
void HelloVulkan::updateDescriptorSet() {
  ...
  VkDescriptorBufferInfo dbiImplDesc{m_implObjects.implBuf.buffer, 0,
                                     VK_WHOLE_SIZE};
  writes.emplace_back(
      m_descSetLayoutBind.makeWrite(m_descSet, 3, &dbiImplDesc));
...
//
void HelloVulkan::createTextureImages(
    const VkCommandBuffer &cmdBuf, const std::vector<std::string> &textures) {
...
void HelloVulkan::onResize(int /*w*/, int /*h*/) {
  ...
  m_raytrace.updateRtDescriptorSet(
...
void HelloVulkan::initRayTracing() {
  ...
  m_raytrace.createRtDescriptorSet(
      m_offscreen.colorTexture().descriptor.imageView);
...
void HelloVulkan::resetFrame() { m_pcRaster.frame = -1; }
...
void HelloVulkan::createImplictBuffers() {
  ...
  m_implObjects.implMatBuf =
      m_alloc.createBuffer(cmdBuf, m_implObjects.implMat,
                           VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
                               VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT);
  cmdGen.submitAndWait(cmdBuf);
  m_alloc.finalizeAndReleaseStaging();
