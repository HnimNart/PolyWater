...
#include <tools/assetc/importers/obj_loader.h>
...
//
void HelloVulkan::setup(const VkInstance& instance, const VkDevice& device,
                        const VkPhysicalDevice& physicalDevice, uint32_t queueFamily)
{
...
void HelloVulkan::updateUniformBuffer(const VkCommandBuffer& cmdBuf)
{
  ...
  glm::mat4 proj =
      glm::perspectiveRH_ZO(glm::radians(CameraManip.getFov()), aspectRatio, 0.1f, 1000.0f);
  ...
  vkCmdPipelineBarrier(cmdBuf, uboUsageStages, VK_PIPELINE_STAGE_TRANSFER_BIT,
                       VK_DEPENDENCY_DEVICE_GROUP_BIT, 0, nullptr, 1, &beforeBarrier, 0, nullptr);
...
void HelloVulkan::createDescriptorSetLayout()
{
  ...
  m_descSetLayoutBind.addBinding(
      SceneBindings::eTextures, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, nbTxt,
...
void HelloVulkan::updateDescriptorSet()
{
  ...
  writes.emplace_back(
      m_descSetLayoutBind.makeWrite(m_descSet, SceneBindings::eObjDescs, &dbiSceneDesc));

  // All texture samplers
  std::vector<VkDescriptorImageInfo> diit;
  diit.reserve(m_textures.size());
  for (auto& texture : m_textures)
  {
    diit.emplace_back(texture.descriptor);
  }
  writes.emplace_back(
      m_descSetLayoutBind.makeWriteArray(m_descSet, SceneBindings::eTextures, diit.data()));
...
void HelloVulkan::createGraphicsPipeline()
{
  ...
  gpb.depthStencilState.depthTestEnable = 1U;
...
void HelloVulkan::loadModel(const std::string& filename, glm::mat4 transform)
{
  ...
  model.matColorBuffer =
      m_alloc.createBuffer(cmdBuf, loader.m_materials, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | flag);
  model.matIndexBuffer =
      m_alloc.createBuffer(cmdBuf, loader.m_matIndx, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | flag);
...
void HelloVulkan::createUniformBuffer()
{
  m_bGlobals = m_alloc.createBuffer(
      sizeof(GlobalUniforms), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
...
void HelloVulkan::createImplictBuffers()
{
  ...
  m_implObjects.implBuf = m_alloc.createBuffer(
      cmdBuf, m_implObjects.objImpl,
